/**
  * \file VecFmncVMProjectState.cpp
  * vector VecFmncVMProjectState (implementation)
...
uint VecFmncVMProjectState::getIx(
			const string& sref
		) {
...
string VecFmncVMProjectState::getSref(
			const uint ix
		) {
...
string VecFmncVMProjectState::getTitle(
			const uint ix
...
void VecFmncVMProjectState::fillFeed(
			const uint ixFmncVLocale
			, Feed& feed
		) {
	feed.clear();
