/**
  * \file PnlFmncSmpList.js
  * web client functionality for panel PnlFmncSmpList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncSmpList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncSmpList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncSmpList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncSmpList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSmpList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncSmpList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncSmpList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSmpList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncSmpList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSmpList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSmpList", "TcoSrfWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSmpList", "TcoSupWidth"));
		var TcoArtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSmpList", "TcoArtWidth"));
		var TcoMatWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSmpList", "TcoMatWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSmpList", "TcoSteWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncSmpList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoArt"); doc.widths.push(TcoArtWidth); doc.fs.push("art"); doc.tos.push(3); wtot += TcoArtWidth;
			} else if (i == 4) {
				doc.cols.push("TcoMat"); doc.widths.push(TcoMatWidth); doc.fs.push("mat"); doc.tos.push(0); wtot += TcoMatWidth;
			} else if (i == 5) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncSmpList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncSmpList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQSmpList
		rownode = getRownode(srcdoc, "ListFmncQSmpList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncSmpList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncSmpList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncSmpList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSmpList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncSmpList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncSmpList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncSmpListData", scrJref, "StgIacQryFmncSmpList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncSmpList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSmpList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncSmpListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncSmpList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncSmpList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncSmpListData", scrJref, "ContIacFmncSmpList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "ContIacFmncSmpList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "ContInfFmncSmpList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "StatShrFmncSmpList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "StgIacFmncSmpList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "TagFmncSmpList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "ListFmncQSmpList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "StatAppQryFmncSmpList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "StatShrQryFmncSmpList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncSmpListData", "StgIacQryFmncSmpList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncSmpListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncSmpListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncSmpListData") {
				mask = mergeDpchEngData(dom);
