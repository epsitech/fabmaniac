/**
  * \file PnlFmncSmpMNStep.js
  * web client functionality for panel PnlFmncSmpMNStep
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncSmpMNStep", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncSmpMNStep", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("MNStep_side").src = "./PnlFmncSmpMNStep_aside.html";
	doc.getElementById("MNStep_cont").src = "./PnlFmncSmpMNStep_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncSmpMNStep", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncSmpMNStep", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("MNStep_side").src = "./PnlFmncSmpMNStep_bside.html";
	doc.getElementById("MNStep_cont").src = "./PnlFmncSmpMNStep_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSmpMNStep", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncSmpMNStep", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncSmpMNStep", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSmpMNStep", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncSmpMNStep", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSmpMNStep", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpMNStep", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpMNStep", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpMNStep", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpMNStep", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpMNStep", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpMNStep", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpMNStep", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSmpMNStep", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncSmpMNStep", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncSmpMNStep", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQSmpMNStep
		rownode = getRownode(srcdoc, "ListFmncQSmpMNStep", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncSmpMNStep", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncSmpMNStep", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncSmpMNStep", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSmpMNStep", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpMNStep", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSmpMNStep", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncSmpMNStep", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncSmpMNStep", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncSmpMNStepData", scrJref, "StgIacQryFmncSmpMNStep");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSmpMNStep", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncSmpMNStep", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSmpMNStep", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncSmpMNStepDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "ContInfFmncSmpMNStep", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "StatAppFmncSmpMNStep", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "StatShrFmncSmpMNStep", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "StgIacFmncSmpMNStep", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "TagFmncSmpMNStep", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "ListFmncQSmpMNStep", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "StatAppQryFmncSmpMNStep", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "StatShrQryFmncSmpMNStep", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncSmpMNStepData", "StgIacQryFmncSmpMNStep", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncSmpMNStepData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncSmpMNStepData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncSmpMNStepData") {
