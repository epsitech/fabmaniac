/**
  * \file CrdFmncSmp.js
  * web client functionality for card CrdFmncSmp
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppFmncSmp", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppFmncSmp", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppFmncSmp", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppFmncSmp", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppFmncSmp", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlFmncSmpHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlFmncSmpList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlFmncSmpRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppFmncSmp", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppFmncSmp", "shortMenu", short);
	setSi(srcdoc, "StatAppFmncSmp", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppFmncSmp", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncSmp", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagFmncSmp", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagFmncSmp", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrFmncSmp", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrFmncSmp", "MitCrdNewAvail") == "true");
	MitCrdGnrActive = (retrieveSi(srcdoc, "StatShrFmncSmp", "MitCrdGnrActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncSmp", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdSmp"), retrieveCi(srcdoc, "ContInfFmncSmp", "MtxCrdSmp"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagFmncSmp", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdGnr"), retrieveTi(srcdoc, "TagFmncSmp", "MitCrdGnr"));
	setMitActive("MitCrdGnr", MitCrdGnrActive);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgFmncSmpGenrep")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlggenrep = retrieveSi(srcdoc, "StatShrFmncSmp", "scrJrefDlggenrep");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrFmncSmp", "scrJrefDlgnew");

	if (scrJrefDlggenrep != "") {
		if (scrJrefDlg != scrJrefDlggenrep) showDlg("DlgFmncSmpGenrep", scrJrefDlggenrep);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgFmncSmpNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfFmncSmp", "MtxCrdSmp") + " - FabManiac 0.1.40";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncSmpDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncSmpDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfFmncSmp", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncSmpData", "ContInfFmncSmp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncSmpData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngFmncSmpData", "StatAppFmncSmp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncSmpData", "StatShrFmncSmp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncSmpData", "TagFmncSmp", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncSmpData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrFmncSmp", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrFmncSmp", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrFmncSmp", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncSmpData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncSmp", "srefIxFmncVReqitmode");

	if (srefIxFmncVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxFmncVReqitmode = "notify";
		setSi(srcdoc, "StatAppFmncSmp", "srefIxFmncVReqitmode", srefIxFmncVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncSmp", "srefIxFmncVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppFmncSmp", "latency"));
