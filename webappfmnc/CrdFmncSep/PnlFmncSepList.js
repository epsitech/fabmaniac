/**
  * \file PnlFmncSepList.js
  * web client functionality for panel PnlFmncSepList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncSepList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncSepList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncSepList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncSepList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSepList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncSepList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncSepList", "srefIxFmncVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfFmncSepList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncSepList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSepList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncSepList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSepList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncSepList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryFmncSepList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSepList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncSepList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncSepList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepList", "TcoTitWidth"));
		var TcoActWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepList", "TcoActWidth"));
		var TcoRltWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepList", "TcoRltWidth"));
		var TcoRluWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepList", "TcoRluWidth"));
		var TcoStpWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepList", "TcoStpWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepList", "TcoStaWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncSepList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoAct"); doc.widths.push(TcoActWidth); doc.fs.push("act"); doc.tos.push(1); wtot += TcoActWidth;
			} else if (i == 3) {
				doc.cols.push("TcoRlt"); doc.widths.push(TcoRltWidth); doc.fs.push("rlt"); doc.tos.push(2); wtot += TcoRltWidth;
			} else if (i == 4) {
				doc.cols.push("TcoRlu"); doc.widths.push(TcoRluWidth); doc.fs.push("rlu"); doc.tos.push(3); wtot += TcoRluWidth;
			} else if (i == 5) {
				doc.cols.push("TcoStp"); doc.widths.push(TcoStpWidth); doc.fs.push("stp"); doc.tos.push(4); wtot += TcoStpWidth;
			} else if (i == 6) {
				doc.cols.push("TcoSta"); doc.widths.push(TcoStaWidth); doc.fs.push("sta"); doc.tos.push(5); wtot += TcoStaWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncSepList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncSepList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQSepList
		rownode = getRownode(srcdoc, "ListFmncQSepList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncSepList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncSepList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncSepList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSepList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncSepList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncSepList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncSepListData", scrJref, "StgIacQryFmncSepList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncSepList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSepList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncSepListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncSepList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncSepList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncSepListData", scrJref, "ContIacFmncSepList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "ContIacFmncSepList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "ContInfFmncSepList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "StatShrFmncSepList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "StgIacFmncSepList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "TagFmncSepList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "ListFmncQSepList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "StatAppQryFmncSepList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "StatShrQryFmncSepList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncSepListData", "StgIacQryFmncSepList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncSepListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncSepListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncSepListData") {
				mask = mergeDpchEngData(dom);
