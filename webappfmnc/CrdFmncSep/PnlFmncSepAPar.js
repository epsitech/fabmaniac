/**
  * \file PnlFmncSepAPar.js
  * web client functionality for panel PnlFmncSepAPar
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncSepAPar", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncSepAPar", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlFmncSepAPar_aside.html";
	doc.getElementById("APar_cont").src = "./PnlFmncSepAPar_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncSepAPar", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncSepAPar", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlFmncSepAPar_bside.html";
	doc.getElementById("APar_cont").src = "./PnlFmncSepAPar_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSepAPar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncSepAPar", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncSepAPar", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncSepAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncSepAPar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSepAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepAPar", "ncol"));

		var TcoSpaWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepAPar", "TcoSpaWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepAPar", "TcoReuWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepAPar", "TcoValWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncSepAPar", "TcoCmtWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoSpa"); doc.widths.push(TcoSpaWidth); doc.fs.push("spa"); wtot += TcoSpaWidth;
			} else if (i == 2) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncSepAPar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncSepAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQSepAPar
		rownode = getRownode(srcdoc, "ListFmncQSepAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncSepAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncSepAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncSepAPar", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSepAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncSepAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncSepAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncSepAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncSepAParData", scrJref, "StgIacQryFmncSepAPar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncSepAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncSepAPar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncSepAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncSepAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "ContInfFmncSepAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "StatAppFmncSepAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "StatShrFmncSepAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "StgIacFmncSepAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "TagFmncSepAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "ListFmncQSepAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "StatAppQryFmncSepAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "StatShrQryFmncSepAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncSepAParData", "StgIacQryFmncSepAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncSepAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncSepAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncSepAParData") {
