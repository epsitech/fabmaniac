/**
  * \file PnlFmncTolSvcMNOrg.js
  * web client functionality for panel PnlFmncTolSvcMNOrg
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolSvcMNOrg", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTolSvcMNOrg", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("SvcMNOrg", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("SvcMNOrg_side").setAttribute("height", "30");
	doc.getElementById("SvcMNOrg_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("SvcMNOrg_side").src = "./PnlFmncTolSvcMNOrg_aside.html";
	doc.getElementById("SvcMNOrg_cont").src = "./PnlFmncTolSvcMNOrg_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolSvcMNOrg", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTolSvcMNOrg", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("SvcMNOrg", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("SvcMNOrg_side").setAttribute("height", "255");
	doc.getElementById("SvcMNOrg_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("SvcMNOrg_side").src = "./PnlFmncTolSvcMNOrg_bside.html";
	doc.getElementById("SvcMNOrg_cont").src = "./PnlFmncTolSvcMNOrg_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("SvcMNOrg_side").contentDocument;
	contdoc = doc.getElementById("SvcMNOrg_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("SvcMNOrg_side").contentDocument;
	contdoc = doc.getElementById("SvcMNOrg_cont").contentDocument;
	hdrdoc = contdoc.getElementById("SvcMNOrg_hdr").contentDocument;
	tbldoc = contdoc.getElementById("SvcMNOrg_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("SvcMNOrg_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolSvcMNOrg", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTolSvcMNOrg", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTolSvcMNOrg", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolSvcMNOrg", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolSvcMNOrg", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolSvcMNOrg", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolSvcMNOrg", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolSvcMNOrg", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolSvcMNOrg", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolSvcMNOrg", "TcoMrefWidth"));
		var TcoTprsWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolSvcMNOrg", "TcoTprsWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTolSvcMNOrg", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTolSvcMNOrg
		rownode = getRownode(srcdoc, "ListFmncQTolSvcMNOrg", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTolSvcMNOrg", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTolSvcMNOrg", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTolSvcMNOrg", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("SvcMNOrg", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("SvcMNOrg_side").setAttribute("height", "" + (height+55));
	doc.getElementById("SvcMNOrg_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("SvcMNOrg_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("SvcMNOrg_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolSvcMNOrg", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolSvcMNOrg", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTolSvcMNOrg", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTolSvcMNOrg", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolSvcMNOrgData", scrJref, "StgIacQryFmncTolSvcMNOrg");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTolSvcMNOrg", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolSvcMNOrg", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTolSvcMNOrgDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "ContInfFmncTolSvcMNOrg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "StatAppFmncTolSvcMNOrg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "StatShrFmncTolSvcMNOrg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "StgIacFmncTolSvcMNOrg", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "TagFmncTolSvcMNOrg", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "ListFmncQTolSvcMNOrg", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "StatAppQryFmncTolSvcMNOrg", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "StatShrQryFmncTolSvcMNOrg", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTolSvcMNOrgData", "StgIacQryFmncTolSvcMNOrg", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTolSvcMNOrgData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTolSvcMNOrgData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("SvcMNOrg");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTolSvcMNOrgData") {
