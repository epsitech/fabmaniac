/**
  * \file PnlFmncTolList.js
  * web client functionality for panel PnlFmncTolList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncTolList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncTolList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncTolList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncTolList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTolList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncTolList", "srefIxFmncVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfFmncTolList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncTolList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncTolList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncTolList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncTolList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolList", "TcoTitWidth"));
		var TcoFabWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolList", "TcoFabWidth"));
		var TcoMnfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolList", "TcoMnfWidth"));
		var TcoUprWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolList", "TcoUprWidth"));
		var TcoTrnWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolList", "TcoTrnWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncTolList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoFab"); doc.widths.push(TcoFabWidth); doc.fs.push("fab"); doc.tos.push(2); wtot += TcoFabWidth;
			} else if (i == 3) {
				doc.cols.push("TcoMnf"); doc.widths.push(TcoMnfWidth); doc.fs.push("mnf"); doc.tos.push(3); wtot += TcoMnfWidth;
			} else if (i == 4) {
				doc.cols.push("TcoUpr"); doc.widths.push(TcoUprWidth); doc.fs.push("upr"); doc.tos.push(0); wtot += TcoUprWidth;
			} else if (i == 5) {
				doc.cols.push("TcoTrn"); doc.widths.push(TcoTrnWidth); doc.fs.push("trn"); doc.tos.push(0); wtot += TcoTrnWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncTolList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTolList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTolList
		rownode = getRownode(srcdoc, "ListFmncQTolList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTolList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTolList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTolList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTolList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTolList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolListData", scrJref, "StgIacQryFmncTolList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTolList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTolListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncTolList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncTolList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolListData", scrJref, "ContIacFmncTolList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "ContIacFmncTolList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "ContInfFmncTolList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "StatShrFmncTolList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "StgIacFmncTolList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "TagFmncTolList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "ListFmncQTolList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "StatAppQryFmncTolList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "StatShrQryFmncTolList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTolListData", "StgIacQryFmncTolList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncTolListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTolListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTolListData") {
				mask = mergeDpchEngData(dom);
