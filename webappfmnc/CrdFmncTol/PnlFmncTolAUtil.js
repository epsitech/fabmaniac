/**
  * \file PnlFmncTolAUtil.js
  * web client functionality for panel PnlFmncTolAUtil
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolAUtil", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTolAUtil", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("AUtil", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AUtil_side").setAttribute("height", "30");
	doc.getElementById("AUtil_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AUtil_side").src = "./PnlFmncTolAUtil_aside.html";
	doc.getElementById("AUtil_cont").src = "./PnlFmncTolAUtil_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolAUtil", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTolAUtil", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("AUtil", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AUtil_side").setAttribute("height", "255");
	doc.getElementById("AUtil_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AUtil_side").src = "./PnlFmncTolAUtil_bside.html";
	doc.getElementById("AUtil_cont").src = "./PnlFmncTolAUtil_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AUtil_side").contentDocument;
	contdoc = doc.getElementById("AUtil_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("AUtil_side").contentDocument;
	contdoc = doc.getElementById("AUtil_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AUtil_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AUtil_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AUtil_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolAUtil", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTolAUtil", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTolAUtil", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolAUtil", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolAUtil", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolAUtil", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAUtil", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAUtil", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAUtil", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAUtil", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAUtil", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAUtil", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAUtil", "ncol"));

		var TcoFpjWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAUtil", "TcoFpjWidth"));
		var TcoFusWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAUtil", "TcoFusWidth"));
		var TcoDatWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAUtil", "TcoDatWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAUtil", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAUtil", "TcoStoWidth"));
		var TcoUprWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAUtil", "TcoUprWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAUtil", "TcoCmtWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoFpj"); doc.widths.push(TcoFpjWidth); doc.fs.push("fpj"); wtot += TcoFpjWidth;
			} else if (i == 2) {
				doc.cols.push("TcoFus"); doc.widths.push(TcoFusWidth); doc.fs.push("fus"); wtot += TcoFusWidth;
			} else if (i == 3) {
				doc.cols.push("TcoDat"); doc.widths.push(TcoDatWidth); doc.fs.push("dat"); wtot += TcoDatWidth;
			} else if (i == 4) {
				...
				doc.cols.push("TcoUpr"); doc.widths.push(TcoUprWidth); doc.fs.push("upr"); wtot += TcoUprWidth;
			} else if (i == 7) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTolAUtil", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTolAUtil", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTolAUtil
		rownode = getRownode(srcdoc, "ListFmncQTolAUtil", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTolAUtil", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTolAUtil", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTolAUtil", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AUtil", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AUtil_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AUtil_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AUtil_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AUtil_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolAUtil", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAUtil", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAUtil", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTolAUtil", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTolAUtil", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolAUtilData", scrJref, "StgIacQryFmncTolAUtil");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAUtil", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTolAUtil", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolAUtil", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTolAUtilDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "ContInfFmncTolAUtil", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "StatAppFmncTolAUtil", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "StatShrFmncTolAUtil", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "StgIacFmncTolAUtil", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "TagFmncTolAUtil", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "ListFmncQTolAUtil", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "StatAppQryFmncTolAUtil", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "StatShrQryFmncTolAUtil", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTolAUtilData", "StgIacQryFmncTolAUtil", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTolAUtilData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTolAUtilData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AUtil");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTolAUtilData") {
