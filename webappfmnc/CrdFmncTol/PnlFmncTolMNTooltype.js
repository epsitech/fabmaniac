/**
  * \file PnlFmncTolMNTooltype.js
  * web client functionality for panel PnlFmncTolMNTooltype
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolMNTooltype", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTolMNTooltype", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("MNTooltype", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNTooltype_side").setAttribute("height", "30");
	doc.getElementById("MNTooltype_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNTooltype_side").src = "./PnlFmncTolMNTooltype_aside.html";
	doc.getElementById("MNTooltype_cont").src = "./PnlFmncTolMNTooltype_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolMNTooltype", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTolMNTooltype", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("MNTooltype", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNTooltype_side").setAttribute("height", "255");
	doc.getElementById("MNTooltype_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNTooltype_side").src = "./PnlFmncTolMNTooltype_bside.html";
	doc.getElementById("MNTooltype_cont").src = "./PnlFmncTolMNTooltype_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNTooltype_side").contentDocument;
	contdoc = doc.getElementById("MNTooltype_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNTooltype_side").contentDocument;
	contdoc = doc.getElementById("MNTooltype_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNTooltype_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNTooltype_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNTooltype_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolMNTooltype", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTolMNTooltype", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTolMNTooltype", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolMNTooltype", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolMNTooltype", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolMNTooltype", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolMNTooltype", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolMNTooltype", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolMNTooltype", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolMNTooltype", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolMNTooltype", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolMNTooltype", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolMNTooltype", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolMNTooltype", "TcoMrefWidth"));
		var TcoTchWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolMNTooltype", "TcoTchWidth"));
		var TcoDefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolMNTooltype", "TcoDefWidth"));
		var TcoOptWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolMNTooltype", "TcoOptWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTolMNTooltype", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTolMNTooltype", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTolMNTooltype
		rownode = getRownode(srcdoc, "ListFmncQTolMNTooltype", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTolMNTooltype", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTolMNTooltype", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTolMNTooltype", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNTooltype", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNTooltype_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNTooltype_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNTooltype_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNTooltype_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolMNTooltype", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolMNTooltype", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolMNTooltype", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTolMNTooltype", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTolMNTooltype", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolMNTooltypeData", scrJref, "StgIacQryFmncTolMNTooltype");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolMNTooltype", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTolMNTooltype", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolMNTooltype", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTolMNTooltypeDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "ContInfFmncTolMNTooltype", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "StatAppFmncTolMNTooltype", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "StatShrFmncTolMNTooltype", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "StgIacFmncTolMNTooltype", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "TagFmncTolMNTooltype", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "ListFmncQTolMNTooltype", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "StatAppQryFmncTolMNTooltype", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "StatShrQryFmncTolMNTooltype", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTolMNTooltypeData", "StgIacQryFmncTolMNTooltype", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTolMNTooltypeData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTolMNTooltypeData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNTooltype");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTolMNTooltypeData") {
