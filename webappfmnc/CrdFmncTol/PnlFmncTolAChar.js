/**
  * \file PnlFmncTolAChar.js
  * web client functionality for panel PnlFmncTolAChar
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolAChar", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTolAChar", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("AChar", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AChar_side").setAttribute("height", "30");
	doc.getElementById("AChar_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AChar_side").src = "./PnlFmncTolAChar_aside.html";
	doc.getElementById("AChar_cont").src = "./PnlFmncTolAChar_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolAChar", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTolAChar", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("AChar", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AChar_side").setAttribute("height", "255");
	doc.getElementById("AChar_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AChar_side").src = "./PnlFmncTolAChar_bside.html";
	doc.getElementById("AChar_cont").src = "./PnlFmncTolAChar_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AChar_side").contentDocument;
	contdoc = doc.getElementById("AChar_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("AChar_side").contentDocument;
	contdoc = doc.getElementById("AChar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AChar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AChar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AChar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolAChar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTolAChar", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTolAChar", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolAChar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolAChar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolAChar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAChar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAChar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAChar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAChar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAChar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAChar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAChar", "ncol"));

		var TcoTchWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAChar", "TcoTchWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAChar", "TcoValWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolAChar", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTolAChar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTolAChar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTolAChar
		rownode = getRownode(srcdoc, "ListFmncQTolAChar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTolAChar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTolAChar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTolAChar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AChar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AChar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AChar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AChar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AChar_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolAChar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAChar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolAChar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTolAChar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTolAChar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolACharData", scrJref, "StgIacQryFmncTolAChar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolAChar", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTolAChar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolAChar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTolACharDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "ContInfFmncTolAChar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "StatAppFmncTolAChar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "StatShrFmncTolAChar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "StgIacFmncTolAChar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "TagFmncTolAChar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "ListFmncQTolAChar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "StatAppQryFmncTolAChar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "StatShrQryFmncTolAChar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTolACharData", "StgIacQryFmncTolAChar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTolACharData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTolACharData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AChar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTolACharData") {
