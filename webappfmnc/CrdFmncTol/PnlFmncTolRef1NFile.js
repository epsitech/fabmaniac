/**
  * \file PnlFmncTolRef1NFile.js
  * web client functionality for panel PnlFmncTolRef1NFile
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolRef1NFile", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTolRef1NFile", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlFmncTolRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlFmncTolRef1NFile_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolRef1NFile", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTolRef1NFile", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlFmncTolRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlFmncTolRef1NFile_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolRef1NFile", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTolRef1NFile", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTolRef1NFile", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolRef1NFile", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolRef1NFile", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTolRef1NFile", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTolRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTolRef1NFile
		rownode = getRownode(srcdoc, "ListFmncQTolRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTolRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTolRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTolRef1NFile", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTolRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTolRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolRef1NFileData", scrJref, "StgIacQryFmncTolRef1NFile");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTolRef1NFile", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTolRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "ContInfFmncTolRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "StatAppFmncTolRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "StatShrFmncTolRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "StgIacFmncTolRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "TagFmncTolRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "ListFmncQTolRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "StatAppQryFmncTolRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "StatShrQryFmncTolRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTolRef1NFileData", "StgIacQryFmncTolRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTolRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTolRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTolRef1NFileData") {
