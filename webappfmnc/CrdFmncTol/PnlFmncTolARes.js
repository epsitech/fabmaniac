/**
  * \file PnlFmncTolARes.js
  * web client functionality for panel PnlFmncTolARes
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolARes", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTolARes", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("ARes", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ARes_side").setAttribute("height", "30");
	doc.getElementById("ARes_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ARes_side").src = "./PnlFmncTolARes_aside.html";
	doc.getElementById("ARes_cont").src = "./PnlFmncTolARes_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTolARes", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTolARes", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("ARes", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ARes_side").setAttribute("height", "255");
	doc.getElementById("ARes_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ARes_side").src = "./PnlFmncTolARes_bside.html";
	doc.getElementById("ARes_cont").src = "./PnlFmncTolARes_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ARes_side").contentDocument;
	contdoc = doc.getElementById("ARes_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("ARes_side").contentDocument;
	contdoc = doc.getElementById("ARes_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ARes_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ARes_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ARes_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolARes", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTolARes", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTolARes", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTolARes", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTolARes", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolARes", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolARes", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolARes", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolARes", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolARes", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolARes", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolARes", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolARes", "ncol"));

		var TcoFusWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolARes", "TcoFusWidth"));
		var TcoDatWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolARes", "TcoDatWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolARes", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolARes", "TcoStoWidth"));
		var TcoRtyWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolARes", "TcoRtyWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTolARes", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTolARes", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTolARes", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTolARes
		rownode = getRownode(srcdoc, "ListFmncQTolARes", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTolARes", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTolARes", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTolARes", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ARes", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ARes_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ARes_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ARes_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ARes_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolARes", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolARes", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTolARes", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTolARes", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTolARes", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTolAResData", scrJref, "StgIacQryFmncTolARes");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTolARes", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTolARes", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTolARes", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTolAResDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "ContInfFmncTolARes", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "StatAppFmncTolARes", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "StatShrFmncTolARes", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "StgIacFmncTolARes", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "TagFmncTolARes", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "ListFmncQTolARes", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "StatAppQryFmncTolARes", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "StatShrQryFmncTolARes", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTolAResData", "StgIacQryFmncTolARes", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTolAResData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTolAResData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ARes");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTolAResData") {
