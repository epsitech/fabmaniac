/**
  * \file PnlFmncPrsList.js
  * web client functionality for panel PnlFmncPrsList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncPrsList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncPrsList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncPrsList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncPrsList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPrsList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncPrsList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncPrsList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPrsList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncPrsList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrsList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrsList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrsList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrsList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrsList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrsList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrsList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrsList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoTitWidth"));
		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoFnmWidth"));
		var TcoLnmWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoLnmWidth"));
		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoOwnWidth"));
		var TcoOrgWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoOrgWidth"));
		var TcoAdrWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoAdrWidth"));
		var TcoSexWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoSexWidth"));
		var TcoTelWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoTelWidth"));
		var TcoEmlWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrsList", "TcoEmlWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncPrsList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoOrg"); doc.widths.push(TcoOrgWidth); doc.fs.push("org"); doc.tos.push(4); wtot += TcoOrgWidth;
			} else if (i == 7) {
				doc.cols.push("TcoAdr"); doc.widths.push(TcoAdrWidth); doc.fs.push("adr"); doc.tos.push(5); wtot += TcoAdrWidth;
			} else if (i == 8) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncPrsList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncPrsList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQPrsList
		rownode = getRownode(srcdoc, "ListFmncQPrsList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncPrsList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncPrsList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncPrsList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrsList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrsList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrsList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncPrsList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncPrsList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPrsListData", scrJref, "StgIacQryFmncPrsList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrsList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncPrsList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrsList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncPrsListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncPrsList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncPrsList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPrsListData", scrJref, "ContIacFmncPrsList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "ContIacFmncPrsList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "ContInfFmncPrsList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "StatShrFmncPrsList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "StgIacFmncPrsList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "TagFmncPrsList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "ListFmncQPrsList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "StatAppQryFmncPrsList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "StatShrQryFmncPrsList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncPrsListData", "StgIacQryFmncPrsList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncPrsListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncPrsListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncPrsListData") {
				mask = mergeDpchEngData(dom);
