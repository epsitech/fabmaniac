/**
  * \file PnlFmncPcoList.js
  * web client functionality for panel PnlFmncPcoList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncPcoList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncPcoList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncPcoList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncPcoList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPcoList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncPcoList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncPcoList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPcoList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncPcoList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPcoList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoList", "TcoSrfWidth"));
		var TcoDatWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoList", "TcoDatWidth"));
		var TcoPcoWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoList", "TcoPcoWidth"));
		var TcoSplWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoList", "TcoSplWidth"));
		var TcoFilWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoList", "TcoFilWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoList", "TcoSteWidth"));
		var TcoAmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoList", "TcoAmtWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncPcoList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoPco"); doc.widths.push(TcoPcoWidth); doc.fs.push("pco"); doc.tos.push(3); wtot += TcoPcoWidth;
			} else if (i == 4) {
				doc.cols.push("TcoSpl"); doc.widths.push(TcoSplWidth); doc.fs.push("spl"); doc.tos.push(4); wtot += TcoSplWidth;
			} else if (i == 5) {
				doc.cols.push("TcoFil"); doc.widths.push(TcoFilWidth); doc.fs.push("fil"); doc.tos.push(5); wtot += TcoFilWidth;
			} else if (i == 6) {
				doc.cols.push("TcoSte"); doc.widths.push(TcoSteWidth); doc.fs.push("ste"); doc.tos.push(6); wtot += TcoSteWidth;
			} else if (i == 7) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncPcoList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncPcoList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQPcoList
		rownode = getRownode(srcdoc, "ListFmncQPcoList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncPcoList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncPcoList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncPcoList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPcoList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncPcoList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncPcoList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPcoListData", scrJref, "StgIacQryFmncPcoList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncPcoList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPcoList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncPcoListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncPcoList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncPcoList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPcoListData", scrJref, "ContIacFmncPcoList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "ContIacFmncPcoList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "ContInfFmncPcoList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "StatShrFmncPcoList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "StgIacFmncPcoList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "TagFmncPcoList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "ListFmncQPcoList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "StatAppQryFmncPcoList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "StatShrQryFmncPcoList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncPcoListData", "StgIacQryFmncPcoList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncPcoListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncPcoListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncPcoListData") {
				mask = mergeDpchEngData(dom);
