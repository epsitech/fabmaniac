/**
  * \file PnlFmncPcoAPos.js
  * web client functionality for panel PnlFmncPcoAPos
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncPcoAPos", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncPcoAPos", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("APos_side").src = "./PnlFmncPcoAPos_aside.html";
	doc.getElementById("APos_cont").src = "./PnlFmncPcoAPos_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncPcoAPos", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncPcoAPos", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("APos_side").src = "./PnlFmncPcoAPos_bside.html";
	doc.getElementById("APos_cont").src = "./PnlFmncPcoAPos_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPcoAPos", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncPcoAPos", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncPcoAPos", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPcoAPos", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncPcoAPos", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPcoAPos", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoAPos", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoAPos", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoAPos", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoAPos", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoAPos", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoAPos", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoAPos", "ncol"));

		var TcoQtyWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoAPos", "TcoQtyWidth"));
		var TcoItnWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoAPos", "TcoItnWidth"));
		var TcoArtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoAPos", "TcoArtWidth"));
		var TcoUprWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoAPos", "TcoUprWidth"));
		var TcoPrcWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPcoAPos", "TcoPrcWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoItn"); doc.widths.push(TcoItnWidth); doc.fs.push("itn"); wtot += TcoItnWidth;
			} else if (i == 3) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncPcoAPos", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncPcoAPos", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQPcoAPos
		rownode = getRownode(srcdoc, "ListFmncQPcoAPos", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncPcoAPos", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncPcoAPos", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncPcoAPos", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPcoAPos", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoAPos", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPcoAPos", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncPcoAPos", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncPcoAPos", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPcoAPosData", scrJref, "StgIacQryFmncPcoAPos");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPcoAPos", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncPcoAPos", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPcoAPos", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncPcoAPosDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "ContInfFmncPcoAPos", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "StatAppFmncPcoAPos", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "StatShrFmncPcoAPos", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "StgIacFmncPcoAPos", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "TagFmncPcoAPos", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "ListFmncQPcoAPos", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "StatAppQryFmncPcoAPos", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "StatShrQryFmncPcoAPos", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncPcoAPosData", "StgIacQryFmncPcoAPos", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncPcoAPosData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncPcoAPosData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncPcoAPosData") {
