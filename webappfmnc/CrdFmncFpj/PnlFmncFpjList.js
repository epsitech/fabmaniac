/**
  * \file PnlFmncFpjList.js
  * web client functionality for panel PnlFmncFpjList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncFpjList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFpjList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncFpjList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFpjList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFpjList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncFpjList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncFpjList", "srefIxFmncVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfFmncFpjList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncFpjList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFpjList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFpjList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFpjList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncFpjList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFpjList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFpjList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFpjList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFpjList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFpjList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFpjList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFpjList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncFpjList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncFpjList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFpjList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFpjList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFpjList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFpjList", "TcoTitWidth"));
		var TcoFabWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFpjList", "TcoFabWidth"));
		var TcoBioWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFpjList", "TcoBioWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFpjList", "TcoSteWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFpjList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoBio"); doc.widths.push(TcoBioWidth); doc.fs.push("bio"); doc.tos.push(3); wtot += TcoBioWidth;
			} else if (i == 5) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncFpjList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncFpjList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQFpjList
		rownode = getRownode(srcdoc, "ListFmncQFpjList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncFpjList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncFpjList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncFpjList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFpjList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFpjList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFpjList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncFpjList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncFpjList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFpjListData", scrJref, "StgIacQryFmncFpjList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFpjList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncFpjList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFpjList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFpjListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFpjList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncFpjList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFpjListData", scrJref, "ContIacFmncFpjList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "ContIacFmncFpjList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "ContInfFmncFpjList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "StatShrFmncFpjList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "StgIacFmncFpjList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "TagFmncFpjList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "ListFmncQFpjList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "StatAppQryFmncFpjList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "StatShrQryFmncFpjList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncFpjListData", "StgIacQryFmncFpjList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncFpjListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncFpjListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncFpjListData") {
				mask = mergeDpchEngData(dom);
