/**
  * \file PnlFmncAdrList.js
  * web client functionality for panel PnlFmncAdrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncAdrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncAdrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncAdrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncAdrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncAdrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncAdrList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncAdrList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncAdrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncAdrList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncAdrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAdrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAdrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAdrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAdrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAdrList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAdrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAdrList", "ncol"));

		var TcoAd1Width = parseInt(retrieveSi(srcdoc, "StgIacFmncAdrList", "TcoAd1Width"));
		var TcoCtyWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncAdrList", "TcoCtyWidth"));
		var TcoHktWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncAdrList", "TcoHktWidth"));
		var TcoHkuWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncAdrList", "TcoHkuWidth"));
		var TcoAtyWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncAdrList", "TcoAtyWidth"));
		var TcoZipWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncAdrList", "TcoZipWidth"));
		var TcoCryWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncAdrList", "TcoCryWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncAdrList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoAd1"); doc.widths.push(TcoAd1Width); doc.fs.push("ad1"); doc.tos.push(0); wtot += TcoAd1Width;
			} else if (i == 2) {
				doc.cols.push("TcoCty"); doc.widths.push(TcoCtyWidth); doc.fs.push("cty"); doc.tos.push(1); wtot += TcoCtyWidth;
			} else if (i == 3) {
				...
				doc.cols.push("TcoZip"); doc.widths.push(TcoZipWidth); doc.fs.push("zip"); doc.tos.push(0); wtot += TcoZipWidth;
			} else if (i == 7) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncAdrList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncAdrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQAdrList
		rownode = getRownode(srcdoc, "ListFmncQAdrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncAdrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncAdrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncAdrList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncAdrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAdrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAdrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncAdrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncAdrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncAdrListData", scrJref, "StgIacQryFmncAdrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAdrList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncAdrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncAdrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncAdrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncAdrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncAdrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncAdrListData", scrJref, "ContIacFmncAdrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "ContIacFmncAdrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "ContInfFmncAdrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "StatShrFmncAdrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "StgIacFmncAdrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "TagFmncAdrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "ListFmncQAdrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "StatAppQryFmncAdrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "StatShrQryFmncAdrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncAdrListData", "StgIacQryFmncAdrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncAdrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncAdrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncAdrListData") {
				mask = mergeDpchEngData(dom);
