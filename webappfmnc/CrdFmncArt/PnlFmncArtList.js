/**
  * \file PnlFmncArtList.js
  * web client functionality for panel PnlFmncArtList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncArtList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncArtList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncArtList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncArtList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncArtList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncArtList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncArtList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncArtList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncArtList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArtList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncArtList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncArtList", "TcoTitWidth"));
		var TcoOrgWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncArtList", "TcoOrgWidth"));
		var TcoTolWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncArtList", "TcoTolWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncArtList", "TcoTypWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncArtList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoOrg"); doc.widths.push(TcoOrgWidth); doc.fs.push("org"); doc.tos.push(2); wtot += TcoOrgWidth;
			} else if (i == 4) {
				doc.cols.push("TcoTol"); doc.widths.push(TcoTolWidth); doc.fs.push("tol"); doc.tos.push(3); wtot += TcoTolWidth;
			} else if (i == 5) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncArtList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncArtList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQArtList
		rownode = getRownode(srcdoc, "ListFmncQArtList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncArtList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncArtList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncArtList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArtList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncArtList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncArtList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncArtListData", scrJref, "StgIacQryFmncArtList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncArtList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArtList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncArtListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncArtList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncArtList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncArtListData", scrJref, "ContIacFmncArtList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "ContIacFmncArtList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "ContInfFmncArtList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "StatShrFmncArtList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "StgIacFmncArtList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "TagFmncArtList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "ListFmncQArtList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "StatAppQryFmncArtList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "StatShrQryFmncArtList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncArtListData", "StgIacQryFmncArtList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncArtListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncArtListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncArtListData") {
				mask = mergeDpchEngData(dom);
