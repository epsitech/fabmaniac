/**
  * \file PnlFmncArtRef1NFile.js
  * web client functionality for panel PnlFmncArtRef1NFile
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncArtRef1NFile", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncArtRef1NFile", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlFmncArtRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlFmncArtRef1NFile_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncArtRef1NFile", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncArtRef1NFile", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlFmncArtRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlFmncArtRef1NFile_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncArtRef1NFile", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncArtRef1NFile", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncArtRef1NFile", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncArtRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncArtRef1NFile", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArtRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncArtRef1NFile", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncArtRef1NFile", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncArtRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQArtRef1NFile
		rownode = getRownode(srcdoc, "ListFmncQArtRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncArtRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncArtRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncArtRef1NFile", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArtRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArtRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncArtRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncArtRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncArtRef1NFileData", scrJref, "StgIacQryFmncArtRef1NFile");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArtRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncArtRef1NFile", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArtRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncArtRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "ContInfFmncArtRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "StatAppFmncArtRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "StatShrFmncArtRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "StgIacFmncArtRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "TagFmncArtRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "ListFmncQArtRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "StatAppQryFmncArtRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "StatShrQryFmncArtRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncArtRef1NFileData", "StgIacQryFmncArtRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncArtRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncArtRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncArtRef1NFileData") {
