/**
  * \file PnlFmncArt1NSample.js
  * web client functionality for panel PnlFmncArt1NSample
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncArt1NSample", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncArt1NSample", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("1NSample", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NSample_side").setAttribute("height", "30");
	doc.getElementById("1NSample_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NSample_side").src = "./PnlFmncArt1NSample_aside.html";
	doc.getElementById("1NSample_cont").src = "./PnlFmncArt1NSample_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncArt1NSample", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncArt1NSample", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("1NSample", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NSample_side").setAttribute("height", "255");
	doc.getElementById("1NSample_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NSample_side").src = "./PnlFmncArt1NSample_bside.html";
	doc.getElementById("1NSample_cont").src = "./PnlFmncArt1NSample_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NSample_side").contentDocument;
	contdoc = doc.getElementById("1NSample_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("1NSample_side").contentDocument;
	contdoc = doc.getElementById("1NSample_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NSample_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NSample_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NSample_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncArt1NSample", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncArt1NSample", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncArt1NSample", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncArt1NSample", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncArt1NSample", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArt1NSample", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArt1NSample", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArt1NSample", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArt1NSample", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArt1NSample", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArt1NSample", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArt1NSample", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArt1NSample", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncArt1NSample", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncArt1NSample", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncArt1NSample", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQArt1NSample
		rownode = getRownode(srcdoc, "ListFmncQArt1NSample", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncArt1NSample", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncArt1NSample", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncArt1NSample", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NSample", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSample_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSample_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NSample_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NSample_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArt1NSample", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArt1NSample", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncArt1NSample", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncArt1NSample", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncArt1NSample", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncArt1NSampleData", scrJref, "StgIacQryFmncArt1NSample");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncArt1NSample", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncArt1NSample", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncArt1NSample", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncArt1NSampleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "ContInfFmncArt1NSample", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "StatAppFmncArt1NSample", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "StatShrFmncArt1NSample", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "StgIacFmncArt1NSample", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "TagFmncArt1NSample", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "ListFmncQArt1NSample", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "StatAppQryFmncArt1NSample", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "StatShrQryFmncArt1NSample", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncArt1NSampleData", "StgIacQryFmncArt1NSample", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncArt1NSampleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncArt1NSampleData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NSample");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncArt1NSampleData") {
