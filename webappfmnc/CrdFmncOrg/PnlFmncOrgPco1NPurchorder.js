/**
  * \file PnlFmncOrgPco1NPurchorder.js
  * web client functionality for panel PnlFmncOrgPco1NPurchorder
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncOrgPco1NPurchorder", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncOrgPco1NPurchorder", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("Pco1NPurchorder", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Pco1NPurchorder_side").setAttribute("height", "30");
	doc.getElementById("Pco1NPurchorder_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Pco1NPurchorder_side").src = "./PnlFmncOrgPco1NPurchorder_aside.html";
	doc.getElementById("Pco1NPurchorder_cont").src = "./PnlFmncOrgPco1NPurchorder_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncOrgPco1NPurchorder", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncOrgPco1NPurchorder", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("Pco1NPurchorder", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Pco1NPurchorder_side").setAttribute("height", "255");
	doc.getElementById("Pco1NPurchorder_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Pco1NPurchorder_side").src = "./PnlFmncOrgPco1NPurchorder_bside.html";
	doc.getElementById("Pco1NPurchorder_cont").src = "./PnlFmncOrgPco1NPurchorder_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Pco1NPurchorder_side").contentDocument;
	contdoc = doc.getElementById("Pco1NPurchorder_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("Pco1NPurchorder_side").contentDocument;
	contdoc = doc.getElementById("Pco1NPurchorder_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Pco1NPurchorder_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Pco1NPurchorder_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Pco1NPurchorder_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrgPco1NPurchorder", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncOrgPco1NPurchorder", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncOrgPco1NPurchorder", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrgPco1NPurchorder", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncOrgPco1NPurchorder", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgPco1NPurchorder", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgPco1NPurchorder", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgPco1NPurchorder", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgPco1NPurchorder", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgPco1NPurchorder", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncOrgPco1NPurchorder", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQOrgPco1NPurchorder
		rownode = getRownode(srcdoc, "ListFmncQOrgPco1NPurchorder", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncOrgPco1NPurchorder", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncOrgPco1NPurchorder", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncOrgPco1NPurchorder", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Pco1NPurchorder", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Pco1NPurchorder_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Pco1NPurchorder_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Pco1NPurchorder_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Pco1NPurchorder_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgPco1NPurchorder", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgPco1NPurchorder", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncOrgPco1NPurchorder", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncOrgPco1NPurchorder", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncOrgPco1NPurchorderData", scrJref, "StgIacQryFmncOrgPco1NPurchorder");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncOrgPco1NPurchorder", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgPco1NPurchorder", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncOrgPco1NPurchorderDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "ContInfFmncOrgPco1NPurchorder", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "StatAppFmncOrgPco1NPurchorder", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "StatShrFmncOrgPco1NPurchorder", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "StgIacFmncOrgPco1NPurchorder", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "TagFmncOrgPco1NPurchorder", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "ListFmncQOrgPco1NPurchorder", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "StatAppQryFmncOrgPco1NPurchorder", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "StatShrQryFmncOrgPco1NPurchorder", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncOrgPco1NPurchorderData", "StgIacQryFmncOrgPco1NPurchorder", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncOrgPco1NPurchorderData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncOrgPco1NPurchorderData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Pco1NPurchorder");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncOrgPco1NPurchorderData") {
