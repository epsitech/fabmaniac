/**
  * \file PnlFmncOrgSup1NOrg.js
  * web client functionality for panel PnlFmncOrgSup1NOrg
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncOrgSup1NOrg", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncOrgSup1NOrg", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("Sup1NOrg", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sup1NOrg_side").setAttribute("height", "30");
	doc.getElementById("Sup1NOrg_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sup1NOrg_side").src = "./PnlFmncOrgSup1NOrg_aside.html";
	doc.getElementById("Sup1NOrg_cont").src = "./PnlFmncOrgSup1NOrg_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncOrgSup1NOrg", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncOrgSup1NOrg", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("Sup1NOrg", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sup1NOrg_side").setAttribute("height", "255");
	doc.getElementById("Sup1NOrg_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sup1NOrg_side").src = "./PnlFmncOrgSup1NOrg_bside.html";
	doc.getElementById("Sup1NOrg_cont").src = "./PnlFmncOrgSup1NOrg_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NOrg_side").contentDocument;
	contdoc = doc.getElementById("Sup1NOrg_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("Sup1NOrg_side").contentDocument;
	contdoc = doc.getElementById("Sup1NOrg_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NOrg_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NOrg_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NOrg_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrgSup1NOrg", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncOrgSup1NOrg", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncOrgSup1NOrg", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrgSup1NOrg", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncOrgSup1NOrg", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgSup1NOrg", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgSup1NOrg", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgSup1NOrg", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgSup1NOrg", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgSup1NOrg", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncOrgSup1NOrg", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQOrgSup1NOrg
		rownode = getRownode(srcdoc, "ListFmncQOrgSup1NOrg", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncOrgSup1NOrg", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncOrgSup1NOrg", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncOrgSup1NOrg", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NOrg", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NOrg_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NOrg_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NOrg_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NOrg_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgSup1NOrg", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgSup1NOrg", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncOrgSup1NOrg", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncOrgSup1NOrg", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncOrgSup1NOrgData", scrJref, "StgIacQryFmncOrgSup1NOrg");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncOrgSup1NOrg", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgSup1NOrg", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncOrgSup1NOrgDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "ContInfFmncOrgSup1NOrg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "StatAppFmncOrgSup1NOrg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "StatShrFmncOrgSup1NOrg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "StgIacFmncOrgSup1NOrg", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "TagFmncOrgSup1NOrg", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "ListFmncQOrgSup1NOrg", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "StatAppQryFmncOrgSup1NOrg", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "StatShrQryFmncOrgSup1NOrg", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncOrgSup1NOrgData", "StgIacQryFmncOrgSup1NOrg", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncOrgSup1NOrgData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncOrgSup1NOrgData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NOrg");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncOrgSup1NOrgData") {
