/**
  * \file CrdFmncOrg.js
  * web client functionality for card CrdFmncOrg
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppFmncOrg", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppFmncOrg", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppFmncOrg", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppFmncOrg", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppFmncOrg", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlFmncOrgHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlFmncOrgList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlFmncOrgRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppFmncOrg", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppFmncOrg", "shortMenu", short);
	setSi(srcdoc, "StatAppFmncOrg", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppFmncOrg", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncOrg", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagFmncOrg", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagFmncOrg", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrFmncOrg", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrFmncOrg", "MitCrdNewAvail") == "true");
	MitCrdImoAvail = (retrieveSi(srcdoc, "StatShrFmncOrg", "MitCrdImoAvail") == "true");
	MitCrdExoActive = (retrieveSi(srcdoc, "StatShrFmncOrg", "MitCrdExoActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncOrg", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdOrg"), retrieveCi(srcdoc, "ContInfFmncOrg", "MtxCrdOrg"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagFmncOrg", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdImo"), retrieveTi(srcdoc, "TagFmncOrg", "MitCrdImo"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdExo"), retrieveTi(srcdoc, "TagFmncOrg", "MitCrdExo"));
	setMitActive("MitCrdExo", MitCrdExoActive);

	height -= setMitMspAvail("MspCrd1", MspCrd1Avail, 1);
	height -= setMitMspAvail("MitCrdNew", MitCrdNewAvail, 20);
	height -= setMitMspAvail("MitCrdImo", MitCrdImoAvail, 20);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgFmncOrgImporg") || (sref == "DlgFmncOrgExporg")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgexporg = retrieveSi(srcdoc, "StatShrFmncOrg", "scrJrefDlgexporg");
	var scrJrefDlgimporg = retrieveSi(srcdoc, "StatShrFmncOrg", "scrJrefDlgimporg");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrFmncOrg", "scrJrefDlgnew");

	if (scrJrefDlgexporg != "") {
		if (scrJrefDlg != scrJrefDlgexporg) showDlg("DlgFmncOrgExporg", scrJrefDlgexporg);
	} else if (scrJrefDlgimporg != "") {
		if (scrJrefDlg != scrJrefDlgimporg) showDlg("DlgFmncOrgImporg", scrJrefDlgimporg);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgFmncOrgNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfFmncOrg", "MtxCrdOrg") + " - FabManiac 0.1.40";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncOrgDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncOrgDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfFmncOrg", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncOrgData", "ContInfFmncOrg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncOrgData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngFmncOrgData", "StatAppFmncOrg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncOrgData", "StatShrFmncOrg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncOrgData", "TagFmncOrg", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncOrgData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrFmncOrg", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrFmncOrg", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrFmncOrg", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncOrgData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncOrg", "srefIxFmncVReqitmode");

	if (srefIxFmncVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxFmncVReqitmode = "notify";
		setSi(srcdoc, "StatAppFmncOrg", "srefIxFmncVReqitmode", srefIxFmncVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncOrg", "srefIxFmncVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppFmncOrg", "latency"));
