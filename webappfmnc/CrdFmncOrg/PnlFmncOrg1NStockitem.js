/**
  * \file PnlFmncOrg1NStockitem.js
  * web client functionality for panel PnlFmncOrg1NStockitem
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncOrg1NStockitem", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncOrg1NStockitem", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("1NStockitem", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NStockitem_side").setAttribute("height", "30");
	doc.getElementById("1NStockitem_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NStockitem_side").src = "./PnlFmncOrg1NStockitem_aside.html";
	doc.getElementById("1NStockitem_cont").src = "./PnlFmncOrg1NStockitem_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncOrg1NStockitem", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncOrg1NStockitem", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("1NStockitem", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NStockitem_side").setAttribute("height", "255");
	doc.getElementById("1NStockitem_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NStockitem_side").src = "./PnlFmncOrg1NStockitem_bside.html";
	doc.getElementById("1NStockitem_cont").src = "./PnlFmncOrg1NStockitem_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NStockitem_side").contentDocument;
	contdoc = doc.getElementById("1NStockitem_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("1NStockitem_side").contentDocument;
	contdoc = doc.getElementById("1NStockitem_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NStockitem_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NStockitem_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NStockitem_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrg1NStockitem", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncOrg1NStockitem", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncOrg1NStockitem", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrg1NStockitem", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncOrg1NStockitem", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrg1NStockitem", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrg1NStockitem", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrg1NStockitem", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrg1NStockitem", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrg1NStockitem", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncOrg1NStockitem", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQOrg1NStockitem
		rownode = getRownode(srcdoc, "ListFmncQOrg1NStockitem", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncOrg1NStockitem", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncOrg1NStockitem", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncOrg1NStockitem", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NStockitem", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NStockitem_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NStockitem_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NStockitem_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NStockitem_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrg1NStockitem", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrg1NStockitem", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncOrg1NStockitem", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncOrg1NStockitem", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncOrg1NStockitemData", scrJref, "StgIacQryFmncOrg1NStockitem");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncOrg1NStockitem", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrg1NStockitem", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncOrg1NStockitemDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "ContInfFmncOrg1NStockitem", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "StatAppFmncOrg1NStockitem", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "StatShrFmncOrg1NStockitem", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "StgIacFmncOrg1NStockitem", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "TagFmncOrg1NStockitem", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "ListFmncQOrg1NStockitem", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "StatAppQryFmncOrg1NStockitem", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "StatShrQryFmncOrg1NStockitem", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncOrg1NStockitemData", "StgIacQryFmncOrg1NStockitem", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncOrg1NStockitemData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncOrg1NStockitemData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NStockitem");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncOrg1NStockitemData") {
