/**
  * \file PnlFmncOrgList.js
  * web client functionality for panel PnlFmncOrgList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncOrgList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncOrgList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncOrgList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncOrgList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrgList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncOrgList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncOrgList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncOrgList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncOrgList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgList", "TcoTitWidth"));
		var TcoDrvWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgList", "TcoDrvWidth"));
		var TcoAdrWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgList", "TcoAdrWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgList", "TcoSupWidth"));
		var TcoTelWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgList", "TcoTelWidth"));
		var TcoEmlWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncOrgList", "TcoEmlWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncOrgList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDrv"); doc.widths.push(TcoDrvWidth); doc.fs.push("drv"); doc.tos.push(3); wtot += TcoDrvWidth;
			} else if (i == 4) {
				doc.cols.push("TcoAdr"); doc.widths.push(TcoAdrWidth); doc.fs.push("adr"); doc.tos.push(4); wtot += TcoAdrWidth;
			} else if (i == 5) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncOrgList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncOrgList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQOrgList
		rownode = getRownode(srcdoc, "ListFmncQOrgList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncOrgList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncOrgList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncOrgList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncOrgList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncOrgList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncOrgList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncOrgListData", scrJref, "StgIacQryFmncOrgList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncOrgList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncOrgList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncOrgList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncOrgListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncOrgList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncOrgList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncOrgListData", scrJref, "ContIacFmncOrgList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "ContIacFmncOrgList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "ContInfFmncOrgList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "StatShrFmncOrgList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "StgIacFmncOrgList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "TagFmncOrgList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "ListFmncQOrgList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "StatAppQryFmncOrgList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "StatShrQryFmncOrgList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncOrgListData", "StgIacQryFmncOrgList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncOrgListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncOrgListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncOrgListData") {
				mask = mergeDpchEngData(dom);
