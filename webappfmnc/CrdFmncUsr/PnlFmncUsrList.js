/**
  * \file PnlFmncUsrList.js
  * web client functionality for panel PnlFmncUsrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncUsrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncUsrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncUsrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncUsrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncUsrList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncUsrList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncUsrList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsrList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsrList", "ncol"));

		var TcoPrsWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsrList", "TcoPrsWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsrList", "TcoSrfWidth"));
		var TcoUsgWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsrList", "TcoUsgWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsrList", "TcoSteWidth"));
		var TcoLclWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsrList", "TcoLclWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsrList", "TcoUlvWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncUsrList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncUsrList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncUsrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQUsrList
		rownode = getRownode(srcdoc, "ListFmncQUsrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncUsrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncUsrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncUsrList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncUsrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncUsrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncUsrListData", scrJref, "StgIacQryFmncUsrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsrList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncUsrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncUsrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncUsrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncUsrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncUsrListData", scrJref, "ContIacFmncUsrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "ContIacFmncUsrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "ContInfFmncUsrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "StatShrFmncUsrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "StgIacFmncUsrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "TagFmncUsrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "ListFmncQUsrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "StatAppQryFmncUsrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "StatShrQryFmncUsrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncUsrListData", "StgIacQryFmncUsrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncUsrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncUsrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncUsrListData") {
				mask = mergeDpchEngData(dom);
