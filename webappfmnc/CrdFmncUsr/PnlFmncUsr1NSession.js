/**
  * \file PnlFmncUsr1NSession.js
  * web client functionality for panel PnlFmncUsr1NSession
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncUsr1NSession", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncUsr1NSession", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlFmncUsr1NSession_aside.html";
	doc.getElementById("1NSession_cont").src = "./PnlFmncUsr1NSession_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncUsr1NSession", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncUsr1NSession", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlFmncUsr1NSession_bside.html";
	doc.getElementById("1NSession_cont").src = "./PnlFmncUsr1NSession_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsr1NSession", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncUsr1NSession", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncUsr1NSession", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsr1NSession", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncUsr1NSession", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsr1NSession", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsr1NSession", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsr1NSession", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsr1NSession", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsr1NSession", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsr1NSession", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsr1NSession", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsr1NSession", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsr1NSession", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncUsr1NSession", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncUsr1NSession", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQUsr1NSession
		rownode = getRownode(srcdoc, "ListFmncQUsr1NSession", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncUsr1NSession", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncUsr1NSession", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncUsr1NSession", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsr1NSession", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsr1NSession", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsr1NSession", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncUsr1NSession", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncUsr1NSession", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncUsr1NSessionData", scrJref, "StgIacQryFmncUsr1NSession");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsr1NSession", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncUsr1NSession", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsr1NSession", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncUsr1NSessionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "ContInfFmncUsr1NSession", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "StatAppFmncUsr1NSession", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "StatShrFmncUsr1NSession", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "StgIacFmncUsr1NSession", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "TagFmncUsr1NSession", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "ListFmncQUsr1NSession", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "StatAppQryFmncUsr1NSession", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "StatShrQryFmncUsr1NSession", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncUsr1NSessionData", "StgIacQryFmncUsr1NSession", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncUsr1NSessionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncUsr1NSessionData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncUsr1NSessionData") {
