/**
  * \file PnlFmncFusList.js
  * web client functionality for panel PnlFmncFusList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncFusList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFusList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncFusList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFusList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFusList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncFusList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncFusList", "srefIxFmncVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfFmncFusList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncFusList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFusList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFusList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFusList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncFusList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFusList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFusList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFusList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFusList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFusList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFusList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFusList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncFusList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncFusList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFusList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFusList", "ncol"));

		var TcoPrsWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFusList", "TcoPrsWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFusList", "TcoSrfWidth"));
		var TcoFabWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFusList", "TcoFabWidth"));
		var TcoFpjWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFusList", "TcoFpjWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFusList", "TcoSteWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFusList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoFab"); doc.widths.push(TcoFabWidth); doc.fs.push("fab"); doc.tos.push(3); wtot += TcoFabWidth;
			} else if (i == 4) {
				doc.cols.push("TcoFpj"); doc.widths.push(TcoFpjWidth); doc.fs.push("fpj"); doc.tos.push(4); wtot += TcoFpjWidth;
			} else if (i == 5) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncFusList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncFusList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQFusList
		rownode = getRownode(srcdoc, "ListFmncQFusList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncFusList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncFusList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncFusList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFusList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFusList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFusList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncFusList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncFusList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFusListData", scrJref, "StgIacQryFmncFusList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFusList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncFusList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFusList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFusListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFusList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncFusList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFusListData", scrJref, "ContIacFmncFusList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "ContIacFmncFusList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "ContInfFmncFusList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "StatShrFmncFusList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "StgIacFmncFusList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "TagFmncFusList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "ListFmncQFusList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "StatAppQryFmncFusList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "StatShrQryFmncFusList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncFusListData", "StgIacQryFmncFusList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncFusListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncFusListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncFusListData") {
				mask = mergeDpchEngData(dom);
