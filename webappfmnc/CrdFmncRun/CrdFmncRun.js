/**
  * \file CrdFmncRun.js
  * web client functionality for card CrdFmncRun
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppFmncRun", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppFmncRun", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppFmncRun", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppFmncRun", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppFmncRun", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlFmncRunHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlFmncRunList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlFmncRunRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppFmncRun", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppFmncRun", "shortMenu", short);
	setSi(srcdoc, "StatAppFmncRun", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppFmncRun", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncRun", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagFmncRun", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagFmncRun", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrFmncRun", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrFmncRun", "MitCrdNewAvail") == "true");
	MitCrdGnrActive = (retrieveSi(srcdoc, "StatShrFmncRun", "MitCrdGnrActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncRun", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdRun"), retrieveCi(srcdoc, "ContInfFmncRun", "MtxCrdRun"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagFmncRun", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdGnr"), retrieveTi(srcdoc, "TagFmncRun", "MitCrdGnr"));
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgFmncRunGenrep")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlggenrep = retrieveSi(srcdoc, "StatShrFmncRun", "scrJrefDlggenrep");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrFmncRun", "scrJrefDlgnew");

	if (scrJrefDlggenrep != "") {
		if (scrJrefDlg != scrJrefDlggenrep) showDlg("DlgFmncRunGenrep", scrJrefDlggenrep);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgFmncRunNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfFmncRun", "MtxCrdRun") + " - FabManiac 0.1.40";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncRunDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncRunDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfFmncRun", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncRunData", "ContInfFmncRun", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncRunData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngFmncRunData", "StatAppFmncRun", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncRunData", "StatShrFmncRun", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncRunData", "TagFmncRun", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncRunData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrFmncRun", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrFmncRun", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrFmncRun", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncRunData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncRun", "srefIxFmncVReqitmode");

	if (srefIxFmncVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxFmncVReqitmode = "notify";
		setSi(srcdoc, "StatAppFmncRun", "srefIxFmncVReqitmode", srefIxFmncVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncRun", "srefIxFmncVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppFmncRun", "latency"));
