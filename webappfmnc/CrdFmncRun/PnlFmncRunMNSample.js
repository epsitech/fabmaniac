/**
  * \file PnlFmncRunMNSample.js
  * web client functionality for panel PnlFmncRunMNSample
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunMNSample", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncRunMNSample", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("MNSample_side").src = "./PnlFmncRunMNSample_aside.html";
	doc.getElementById("MNSample_cont").src = "./PnlFmncRunMNSample_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunMNSample", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncRunMNSample", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("MNSample_side").src = "./PnlFmncRunMNSample_bside.html";
	doc.getElementById("MNSample_cont").src = "./PnlFmncRunMNSample_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunMNSample", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncRunMNSample", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncRunMNSample", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunMNSample", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncRunMNSample", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNSample", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNSample", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNSample", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNSample", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNSample", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNSample", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNSample", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNSample", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncRunMNSample", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncRunMNSample", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncRunMNSample", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQRunMNSample
		rownode = getRownode(srcdoc, "ListFmncQRunMNSample", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncRunMNSample", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncRunMNSample", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncRunMNSample", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNSample", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNSample", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNSample", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncRunMNSample", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncRunMNSample", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncRunMNSampleData", scrJref, "StgIacQryFmncRunMNSample");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNSample", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncRunMNSample", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNSample", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncRunMNSampleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "ContInfFmncRunMNSample", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "StatAppFmncRunMNSample", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "StatShrFmncRunMNSample", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "StgIacFmncRunMNSample", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "TagFmncRunMNSample", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "ListFmncQRunMNSample", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "StatAppQryFmncRunMNSample", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "StatShrQryFmncRunMNSample", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncRunMNSampleData", "StgIacQryFmncRunMNSample", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncRunMNSampleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncRunMNSampleData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncRunMNSampleData") {
