/**
  * \file PnlFmncRunMNFab.js
  * web client functionality for panel PnlFmncRunMNFab
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunMNFab", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncRunMNFab", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("MNFab", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNFab_side").setAttribute("height", "30");
	doc.getElementById("MNFab_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNFab_side").src = "./PnlFmncRunMNFab_aside.html";
	doc.getElementById("MNFab_cont").src = "./PnlFmncRunMNFab_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunMNFab", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncRunMNFab", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("MNFab", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNFab_side").setAttribute("height", "255");
	doc.getElementById("MNFab_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNFab_side").src = "./PnlFmncRunMNFab_bside.html";
	doc.getElementById("MNFab_cont").src = "./PnlFmncRunMNFab_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNFab_side").contentDocument;
	contdoc = doc.getElementById("MNFab_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNFab_side").contentDocument;
	contdoc = doc.getElementById("MNFab_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNFab_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNFab_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNFab_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunMNFab", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncRunMNFab", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncRunMNFab", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunMNFab", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncRunMNFab", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNFab", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNFab", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNFab", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNFab", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNFab", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNFab", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNFab", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNFab", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncRunMNFab", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncRunMNFab", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncRunMNFab", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQRunMNFab
		rownode = getRownode(srcdoc, "ListFmncQRunMNFab", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncRunMNFab", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncRunMNFab", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncRunMNFab", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNFab", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNFab_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNFab_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNFab_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNFab_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNFab", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNFab", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNFab", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncRunMNFab", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncRunMNFab", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncRunMNFabData", scrJref, "StgIacQryFmncRunMNFab");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNFab", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncRunMNFab", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNFab", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncRunMNFabDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "ContInfFmncRunMNFab", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "StatAppFmncRunMNFab", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "StatShrFmncRunMNFab", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "StgIacFmncRunMNFab", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "TagFmncRunMNFab", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "ListFmncQRunMNFab", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "StatAppQryFmncRunMNFab", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "StatShrQryFmncRunMNFab", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncRunMNFabData", "StgIacQryFmncRunMNFab", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncRunMNFabData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncRunMNFabData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNFab");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncRunMNFabData") {
