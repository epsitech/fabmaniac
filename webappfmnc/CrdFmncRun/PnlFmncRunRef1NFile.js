/**
  * \file PnlFmncRunRef1NFile.js
  * web client functionality for panel PnlFmncRunRef1NFile
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunRef1NFile", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncRunRef1NFile", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlFmncRunRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlFmncRunRef1NFile_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunRef1NFile", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncRunRef1NFile", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlFmncRunRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlFmncRunRef1NFile_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunRef1NFile", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncRunRef1NFile", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncRunRef1NFile", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncRunRef1NFile", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncRunRef1NFile", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncRunRef1NFile", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncRunRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQRunRef1NFile
		rownode = getRownode(srcdoc, "ListFmncQRunRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncRunRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncRunRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncRunRef1NFile", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncRunRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncRunRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncRunRef1NFileData", scrJref, "StgIacQryFmncRunRef1NFile");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncRunRef1NFile", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncRunRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "ContInfFmncRunRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "StatAppFmncRunRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "StatShrFmncRunRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "StgIacFmncRunRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "TagFmncRunRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "ListFmncQRunRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "StatAppQryFmncRunRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "StatShrQryFmncRunRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncRunRef1NFileData", "StgIacQryFmncRunRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncRunRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncRunRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncRunRef1NFileData") {
