/**
  * \file PnlFmncRunMNPerson.js
  * web client functionality for panel PnlFmncRunMNPerson
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunMNPerson", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncRunMNPerson", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("MNPerson_side").src = "./PnlFmncRunMNPerson_aside.html";
	doc.getElementById("MNPerson_cont").src = "./PnlFmncRunMNPerson_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncRunMNPerson", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncRunMNPerson", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("MNPerson_side").src = "./PnlFmncRunMNPerson_bside.html";
	doc.getElementById("MNPerson_cont").src = "./PnlFmncRunMNPerson_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunMNPerson", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncRunMNPerson", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncRunMNPerson", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncRunMNPerson", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncRunMNPerson", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNPerson", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNPerson", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNPerson", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNPerson", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNPerson", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNPerson", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNPerson", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNPerson", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncRunMNPerson", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncRunMNPerson", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncRunMNPerson", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQRunMNPerson
		rownode = getRownode(srcdoc, "ListFmncQRunMNPerson", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncRunMNPerson", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncRunMNPerson", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncRunMNPerson", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNPerson", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNPerson", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncRunMNPerson", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncRunMNPerson", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncRunMNPerson", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncRunMNPersonData", scrJref, "StgIacQryFmncRunMNPerson");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncRunMNPerson", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncRunMNPerson", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncRunMNPerson", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncRunMNPersonDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "ContInfFmncRunMNPerson", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "StatAppFmncRunMNPerson", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "StatShrFmncRunMNPerson", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "StgIacFmncRunMNPerson", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "TagFmncRunMNPerson", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "ListFmncQRunMNPerson", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "StatAppQryFmncRunMNPerson", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "StatShrQryFmncRunMNPerson", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncRunMNPersonData", "StgIacQryFmncRunMNPerson", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncRunMNPersonData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncRunMNPersonData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncRunMNPersonData") {
