/**
  * \file CrdFmncPrj.js
  * web client functionality for card CrdFmncPrj
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppFmncPrj", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppFmncPrj", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppFmncPrj", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppFmncPrj", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppFmncPrj", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlFmncPrjHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlFmncPrjList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlFmncPrjRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppFmncPrj", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppFmncPrj", "shortMenu", short);
	setSi(srcdoc, "StatAppFmncPrj", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppFmncPrj", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncPrj", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagFmncPrj", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagFmncPrj", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrFmncPrj", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrFmncPrj", "MitCrdNewAvail") == "true");
	MitCrdImeAvail = (retrieveSi(srcdoc, "StatShrFmncPrj", "MitCrdImeAvail") == "true");
	MitCrdExeActive = (retrieveSi(srcdoc, "StatShrFmncPrj", "MitCrdExeActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncPrj", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdPrj"), retrieveCi(srcdoc, "ContInfFmncPrj", "MtxCrdPrj"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagFmncPrj", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdIme"), retrieveTi(srcdoc, "TagFmncPrj", "MitCrdIme"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdExe"), retrieveTi(srcdoc, "TagFmncPrj", "MitCrdExe"));
	setMitActive("MitCrdExe", MitCrdExeActive);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgFmncPrjImpelb") || (sref == "DlgFmncPrjExpelb")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgexpelb = retrieveSi(srcdoc, "StatShrFmncPrj", "scrJrefDlgexpelb");
	var scrJrefDlgimpelb = retrieveSi(srcdoc, "StatShrFmncPrj", "scrJrefDlgimpelb");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrFmncPrj", "scrJrefDlgnew");

	if (scrJrefDlgexpelb != "") {
		if (scrJrefDlg != scrJrefDlgexpelb) showDlg("DlgFmncPrjExpelb", scrJrefDlgexpelb);
	} else if (scrJrefDlgimpelb != "") {
		if (scrJrefDlg != scrJrefDlgimpelb) showDlg("DlgFmncPrjImpelb", scrJrefDlgimpelb);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgFmncPrjNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfFmncPrj", "MtxCrdPrj") + " - FabManiac 0.1.40";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncPrjDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncPrjDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfFmncPrj", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncPrjData", "ContInfFmncPrj", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncPrjData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngFmncPrjData", "StatAppFmncPrj", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncPrjData", "StatShrFmncPrj", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncPrjData", "TagFmncPrj", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncPrjData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrFmncPrj", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrFmncPrj", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrFmncPrj", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncPrjData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncPrj", "srefIxFmncVReqitmode");

	if (srefIxFmncVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxFmncVReqitmode = "notify";
		setSi(srcdoc, "StatAppFmncPrj", "srefIxFmncVReqitmode", srefIxFmncVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncPrj", "srefIxFmncVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppFmncPrj", "latency"));
