/**
  * \file PnlFmncPrj1NRun.js
  * web client functionality for panel PnlFmncPrj1NRun
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncPrj1NRun", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncPrj1NRun", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("1NRun", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NRun_side").setAttribute("height", "30");
	doc.getElementById("1NRun_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NRun_side").src = "./PnlFmncPrj1NRun_aside.html";
	doc.getElementById("1NRun_cont").src = "./PnlFmncPrj1NRun_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncPrj1NRun", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncPrj1NRun", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("1NRun", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NRun_side").setAttribute("height", "255");
	doc.getElementById("1NRun_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NRun_side").src = "./PnlFmncPrj1NRun_bside.html";
	doc.getElementById("1NRun_cont").src = "./PnlFmncPrj1NRun_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NRun_side").contentDocument;
	contdoc = doc.getElementById("1NRun_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("1NRun_side").contentDocument;
	contdoc = doc.getElementById("1NRun_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NRun_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NRun_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NRun_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPrj1NRun", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncPrj1NRun", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncPrj1NRun", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPrj1NRun", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncPrj1NRun", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrj1NRun", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrj1NRun", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrj1NRun", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrj1NRun", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrj1NRun", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrj1NRun", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrj1NRun", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrj1NRun", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrj1NRun", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncPrj1NRun", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncPrj1NRun", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQPrj1NRun
		rownode = getRownode(srcdoc, "ListFmncQPrj1NRun", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncPrj1NRun", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncPrj1NRun", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncPrj1NRun", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NRun", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NRun_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NRun_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NRun_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NRun_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrj1NRun", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrj1NRun", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrj1NRun", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncPrj1NRun", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncPrj1NRun", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPrj1NRunData", scrJref, "StgIacQryFmncPrj1NRun");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrj1NRun", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncPrj1NRun", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrj1NRun", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncPrj1NRunDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "ContInfFmncPrj1NRun", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "StatAppFmncPrj1NRun", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "StatShrFmncPrj1NRun", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "StgIacFmncPrj1NRun", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "TagFmncPrj1NRun", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "ListFmncQPrj1NRun", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "StatAppQryFmncPrj1NRun", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "StatShrQryFmncPrj1NRun", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncPrj1NRunData", "StgIacQryFmncPrj1NRun", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncPrj1NRunData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncPrj1NRunData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NRun");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncPrj1NRunData") {
