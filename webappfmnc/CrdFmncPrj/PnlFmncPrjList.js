/**
  * \file PnlFmncPrjList.js
  * web client functionality for panel PnlFmncPrjList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncPrjList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncPrjList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncPrjList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncPrjList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPrjList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncPrjList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncPrjList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncPrjList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncPrjList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrjList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrjList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrjList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrjList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrjList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrjList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrjList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrjList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrjList", "TcoTitWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncPrjList", "TcoSteWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncPrjList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSte"); doc.widths.push(TcoSteWidth); doc.fs.push("ste"); doc.tos.push(2); wtot += TcoSteWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncPrjList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncPrjList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQPrjList
		rownode = getRownode(srcdoc, "ListFmncQPrjList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncPrjList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncPrjList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncPrjList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrjList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrjList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncPrjList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncPrjList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncPrjList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPrjListData", scrJref, "StgIacQryFmncPrjList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncPrjList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncPrjList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncPrjList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncPrjListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncPrjList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncPrjList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncPrjListData", scrJref, "ContIacFmncPrjList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "ContIacFmncPrjList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "ContInfFmncPrjList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "StatShrFmncPrjList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "StgIacFmncPrjList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "TagFmncPrjList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "ListFmncQPrjList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "StatAppQryFmncPrjList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "StatShrQryFmncPrjList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncPrjListData", "StgIacQryFmncPrjList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncPrjListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncPrjListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncPrjListData") {
				mask = mergeDpchEngData(dom);
