/**
  * \file PnlFmncFilList.js
  * web client functionality for panel PnlFmncFilList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncFilList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFilList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncFilList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFilList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFilList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncFilList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncFilList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFilList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFilList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilList", "ncol"));

		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilList", "TcoFnmWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilList", "TcoReuWidth"));
		var TcoCntWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilList", "TcoCntWidth"));
		var TcoMimWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilList", "TcoMimWidth"));
		var TcoSizWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilList", "TcoSizWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFilList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncFilList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncFilList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQFilList
		rownode = getRownode(srcdoc, "ListFmncQFilList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncFilList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncFilList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncFilList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncFilList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncFilList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFilListData", scrJref, "StgIacQryFmncFilList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncFilList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFilListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFilList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncFilList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFilListData", scrJref, "ContIacFmncFilList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "ContIacFmncFilList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "ContInfFmncFilList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "StatShrFmncFilList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "StgIacFmncFilList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "TagFmncFilList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "ListFmncQFilList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "StatAppQryFmncFilList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "StatShrQryFmncFilList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncFilListData", "StgIacQryFmncFilList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncFilListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncFilListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncFilListData") {
				mask = mergeDpchEngData(dom);
