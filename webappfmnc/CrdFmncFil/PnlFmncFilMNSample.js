/**
  * \file PnlFmncFilMNSample.js
  * web client functionality for panel PnlFmncFilMNSample
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncFilMNSample", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncFilMNSample", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("MNSample", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNSample_side").setAttribute("height", "30");
	doc.getElementById("MNSample_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNSample_side").src = "./PnlFmncFilMNSample_aside.html";
	doc.getElementById("MNSample_cont").src = "./PnlFmncFilMNSample_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncFilMNSample", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncFilMNSample", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("MNSample", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNSample_side").setAttribute("height", "255");
	doc.getElementById("MNSample_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNSample_side").src = "./PnlFmncFilMNSample_bside.html";
	doc.getElementById("MNSample_cont").src = "./PnlFmncFilMNSample_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNSample_side").contentDocument;
	contdoc = doc.getElementById("MNSample_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNSample_side").contentDocument;
	contdoc = doc.getElementById("MNSample_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNSample_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNSample_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNSample_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFilMNSample", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncFilMNSample", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncFilMNSample", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFilMNSample", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFilMNSample", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilMNSample", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNSample", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNSample", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNSample", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNSample", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNSample", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNSample", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNSample", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilMNSample", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncFilMNSample", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncFilMNSample", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQFilMNSample
		rownode = getRownode(srcdoc, "ListFmncQFilMNSample", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncFilMNSample", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncFilMNSample", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncFilMNSample", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNSample", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNSample_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNSample_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNSample_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNSample_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilMNSample", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNSample", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNSample", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncFilMNSample", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncFilMNSample", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFilMNSampleData", scrJref, "StgIacQryFmncFilMNSample");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNSample", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncFilMNSample", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilMNSample", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFilMNSampleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "ContInfFmncFilMNSample", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "StatAppFmncFilMNSample", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "StatShrFmncFilMNSample", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "StgIacFmncFilMNSample", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "TagFmncFilMNSample", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "ListFmncQFilMNSample", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "StatAppQryFmncFilMNSample", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "StatShrQryFmncFilMNSample", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncFilMNSampleData", "StgIacQryFmncFilMNSample", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncFilMNSampleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncFilMNSampleData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNSample");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncFilMNSampleData") {
