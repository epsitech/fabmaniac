/**
  * \file PnlFmncFilMNStep.js
  * web client functionality for panel PnlFmncFilMNStep
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncFilMNStep", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncFilMNStep", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("MNStep", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNStep_side").setAttribute("height", "30");
	doc.getElementById("MNStep_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNStep_side").src = "./PnlFmncFilMNStep_aside.html";
	doc.getElementById("MNStep_cont").src = "./PnlFmncFilMNStep_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncFilMNStep", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncFilMNStep", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("MNStep", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNStep_side").setAttribute("height", "255");
	doc.getElementById("MNStep_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNStep_side").src = "./PnlFmncFilMNStep_bside.html";
	doc.getElementById("MNStep_cont").src = "./PnlFmncFilMNStep_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNStep_side").contentDocument;
	contdoc = doc.getElementById("MNStep_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNStep_side").contentDocument;
	contdoc = doc.getElementById("MNStep_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNStep_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNStep_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNStep_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFilMNStep", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncFilMNStep", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncFilMNStep", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFilMNStep", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFilMNStep", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilMNStep", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNStep", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNStep", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNStep", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNStep", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNStep", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNStep", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNStep", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilMNStep", "TcoMrefWidth"));
		var TcoTsmpWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilMNStep", "TcoTsmpWidth"));
		var TcoDirWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFilMNStep", "TcoDirWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTsmp"); doc.widths.push(TcoTsmpWidth); doc.fs.push("tsmp"); wtot += TcoTsmpWidth;
			} else if (i == 3) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncFilMNStep", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncFilMNStep", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQFilMNStep
		rownode = getRownode(srcdoc, "ListFmncQFilMNStep", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncFilMNStep", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncFilMNStep", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncFilMNStep", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNStep", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNStep_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNStep_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNStep_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNStep_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilMNStep", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNStep", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFilMNStep", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncFilMNStep", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncFilMNStep", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFilMNStepData", scrJref, "StgIacQryFmncFilMNStep");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFilMNStep", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncFilMNStep", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFilMNStep", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFilMNStepDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "ContInfFmncFilMNStep", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "StatAppFmncFilMNStep", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "StatShrFmncFilMNStep", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "StgIacFmncFilMNStep", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "TagFmncFilMNStep", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "ListFmncQFilMNStep", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "StatAppQryFmncFilMNStep", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "StatShrQryFmncFilMNStep", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncFilMNStepData", "StgIacQryFmncFilMNStep", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncFilMNStepData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncFilMNStepData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNStep");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncFilMNStepData") {
