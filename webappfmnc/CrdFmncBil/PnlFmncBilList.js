/**
  * \file PnlFmncBilList.js
  * web client functionality for panel PnlFmncBilList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncBilList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncBilList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncBilList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncBilList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncBilList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncBilList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncBilList", "srefIxFmncVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfFmncBilList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncBilList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncBilList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncBilList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncBilList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncBilList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryFmncBilList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncBilList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagFmncBilList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfFmncBilList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoSrfWidth"));
		var TcoDatWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoDatWidth"));
		var TcoPrjWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoPrjWidth"));
		var TcoFilWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoFilWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoSteWidth"));
		var TcoPraWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoPraWidth"));
		var TcoProWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoProWidth"));
		var TcoAmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilList", "TcoAmtWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncBilList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDat"); doc.widths.push(TcoDatWidth); doc.fs.push("dat"); doc.tos.push(2); wtot += TcoDatWidth;
			} else if (i == 3) {
				doc.cols.push("TcoPrj"); doc.widths.push(TcoPrjWidth); doc.fs.push("prj"); doc.tos.push(3); wtot += TcoPrjWidth;
			} else if (i == 4) {
				doc.cols.push("TcoFil"); doc.widths.push(TcoFilWidth); doc.fs.push("fil"); doc.tos.push(4); wtot += TcoFilWidth;
			} else if (i == 5) {
				doc.cols.push("TcoSte"); doc.widths.push(TcoSteWidth); doc.fs.push("ste"); doc.tos.push(5); wtot += TcoSteWidth;
			} else if (i == 6) {
				doc.cols.push("TcoPra"); doc.widths.push(TcoPraWidth); doc.fs.push("pra"); doc.tos.push(0); wtot += TcoPraWidth;
			} else if (i == 7) {
				doc.cols.push("TcoPro"); doc.widths.push(TcoProWidth); doc.fs.push("pro"); doc.tos.push(0); wtot += TcoProWidth;
			} else if (i == 8) {
				doc.cols.push("TcoAmt"); doc.widths.push(TcoAmtWidth); doc.fs.push("amt"); doc.tos.push(0); wtot += TcoAmtWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncBilList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncBilList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQBilList
		rownode = getRownode(srcdoc, "ListFmncQBilList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncBilList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncBilList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncBilList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncBilList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncBilList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncBilList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncBilListData", scrJref, "StgIacQryFmncBilList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncBilList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncBilList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncBilListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncBilList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncBilList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncBilListData", scrJref, "ContIacFmncBilList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "ContIacFmncBilList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "ContInfFmncBilList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "StatShrFmncBilList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "StgIacFmncBilList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "TagFmncBilList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "ListFmncQBilList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "StatAppQryFmncBilList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "StatShrQryFmncBilList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncBilListData", "StgIacQryFmncBilList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncBilListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncBilListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncBilListData") {
				mask = mergeDpchEngData(dom);
