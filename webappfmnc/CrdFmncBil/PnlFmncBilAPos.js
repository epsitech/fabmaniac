/**
  * \file PnlFmncBilAPos.js
  * web client functionality for panel PnlFmncBilAPos
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncBilAPos", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncBilAPos", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("APos", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("APos_side").setAttribute("height", "30");
	doc.getElementById("APos_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("APos_side").src = "./PnlFmncBilAPos_aside.html";
	doc.getElementById("APos_cont").src = "./PnlFmncBilAPos_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncBilAPos", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncBilAPos", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("APos", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("APos_side").setAttribute("height", "255");
	doc.getElementById("APos_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("APos_side").src = "./PnlFmncBilAPos_bside.html";
	doc.getElementById("APos_cont").src = "./PnlFmncBilAPos_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("APos_side").contentDocument;
	contdoc = doc.getElementById("APos_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("APos_side").contentDocument;
	contdoc = doc.getElementById("APos_cont").contentDocument;
	hdrdoc = contdoc.getElementById("APos_hdr").contentDocument;
	tbldoc = contdoc.getElementById("APos_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("APos_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncBilAPos", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncBilAPos", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncBilAPos", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncBilAPos", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncBilAPos", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncBilAPos", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilAPos", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilAPos", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilAPos", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilAPos", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilAPos", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilAPos", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilAPos", "ncol"));

		var TcoFusWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilAPos", "TcoFusWidth"));
		var TcoTolWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilAPos", "TcoTolWidth"));
		var TcoQtyWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilAPos", "TcoQtyWidth"));
		var TcoArtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilAPos", "TcoArtWidth"));
		var TcoUprWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilAPos", "TcoUprWidth"));
		var TcoPrcWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncBilAPos", "TcoPrcWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoQty"); doc.widths.push(TcoQtyWidth); doc.fs.push("qty"); wtot += TcoQtyWidth;
			} else if (i == 4) {
				doc.cols.push("TcoArt"); doc.widths.push(TcoArtWidth); doc.fs.push("art"); wtot += TcoArtWidth;
			} else if (i == 5) {
				doc.cols.push("TcoUpr"); doc.widths.push(TcoUprWidth); doc.fs.push("upr"); wtot += TcoUprWidth;
			} else if (i == 6) {
				doc.cols.push("TcoPrc"); doc.widths.push(TcoPrcWidth); doc.fs.push("prc"); wtot += TcoPrcWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncBilAPos", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncBilAPos", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQBilAPos
		rownode = getRownode(srcdoc, "ListFmncQBilAPos", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncBilAPos", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncBilAPos", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncBilAPos", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("APos", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("APos_side").setAttribute("height", "" + (height+55));
	doc.getElementById("APos_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("APos_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("APos_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncBilAPos", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilAPos", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncBilAPos", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncBilAPos", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncBilAPos", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncBilAPosData", scrJref, "StgIacQryFmncBilAPos");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncBilAPos", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncBilAPos", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncBilAPos", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncBilAPosDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "ContInfFmncBilAPos", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "StatAppFmncBilAPos", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "StatShrFmncBilAPos", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "StgIacFmncBilAPos", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "TagFmncBilAPos", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "ListFmncQBilAPos", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "StatAppQryFmncBilAPos", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "StatShrQryFmncBilAPos", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncBilAPosData", "StgIacQryFmncBilAPos", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncBilAPosData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncBilAPosData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("APos");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncBilAPosData") {
