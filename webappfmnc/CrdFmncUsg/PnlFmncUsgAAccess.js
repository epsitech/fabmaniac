/**
  * \file PnlFmncUsgAAccess.js
  * web client functionality for panel PnlFmncUsgAAccess
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncUsgAAccess", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncUsgAAccess", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlFmncUsgAAccess_aside.html";
	doc.getElementById("AAccess_cont").src = "./PnlFmncUsgAAccess_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncUsgAAccess", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncUsgAAccess", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlFmncUsgAAccess_bside.html";
	doc.getElementById("AAccess_cont").src = "./PnlFmncUsgAAccess_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsgAAccess", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncUsgAAccess", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncUsgAAccess", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsgAAccess", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncUsgAAccess", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgAAccess", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgAAccess", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgAAccess", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgAAccess", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgAAccess", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgAAccess", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgAAccess", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgAAccess", "ncol"));

		var TcoCrdWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsgAAccess", "TcoCrdWidth"));
		var TcoUacWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsgAAccess", "TcoUacWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncUsgAAccess", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncUsgAAccess", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQUsgAAccess
		rownode = getRownode(srcdoc, "ListFmncQUsgAAccess", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncUsgAAccess", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncUsgAAccess", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncUsgAAccess", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgAAccess", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgAAccess", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgAAccess", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncUsgAAccess", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncUsgAAccess", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncUsgAAccessData", scrJref, "StgIacQryFmncUsgAAccess");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgAAccess", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncUsgAAccess", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgAAccess", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncUsgAAccessDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "ContInfFmncUsgAAccess", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "StatAppFmncUsgAAccess", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "StatShrFmncUsgAAccess", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "StgIacFmncUsgAAccess", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "TagFmncUsgAAccess", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "ListFmncQUsgAAccess", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "StatAppQryFmncUsgAAccess", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "StatShrQryFmncUsgAAccess", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncUsgAAccessData", "StgIacQryFmncUsgAAccess", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncUsgAAccessData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncUsgAAccessData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncUsgAAccessData") {
