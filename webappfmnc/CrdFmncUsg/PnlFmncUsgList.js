/**
  * \file PnlFmncUsgList.js
  * web client functionality for panel PnlFmncUsgList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncUsgList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncUsgList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncUsgList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncUsgList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsgList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncUsgList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncUsgList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsgList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncUsgList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsgList", "TcoSrfWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncUsgList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncUsgList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncUsgList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQUsgList
		rownode = getRownode(srcdoc, "ListFmncQUsgList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncUsgList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncUsgList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncUsgList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncUsgList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncUsgList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncUsgListData", scrJref, "StgIacQryFmncUsgList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncUsgList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncUsgListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncUsgList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncUsgList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncUsgListData", scrJref, "ContIacFmncUsgList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "ContIacFmncUsgList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "ContInfFmncUsgList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "StatShrFmncUsgList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "StgIacFmncUsgList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "TagFmncUsgList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "ListFmncQUsgList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "StatAppQryFmncUsgList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "StatShrQryFmncUsgList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncUsgListData", "StgIacQryFmncUsgList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncUsgListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncUsgListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncUsgListData") {
				mask = mergeDpchEngData(dom);
