/**
  * \file PnlFmncUsgMNUser.js
  * web client functionality for panel PnlFmncUsgMNUser
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncUsgMNUser", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncUsgMNUser", "srefIxFmncVExpstate", "mind");
...
// IP minimize.estapp --- END
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlFmncUsgMNUser_aside.html";
	doc.getElementById("MNUser_cont").src = "./PnlFmncUsgMNUser_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncUsgMNUser", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncUsgMNUser", "srefIxFmncVExpstate", "regd");
...
// IP regularize.estapp --- END
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlFmncUsgMNUser_bside.html";
	doc.getElementById("MNUser_cont").src = "./PnlFmncUsgMNUser_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsgMNUser", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncUsgMNUser", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncUsgMNUser", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncUsgMNUser", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncUsgMNUser", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgMNUser", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgMNUser", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgMNUser", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgMNUser", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgMNUser", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgMNUser", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgMNUser", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgMNUser", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsgMNUser", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncUsgMNUser", "TcoUlvWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncUsgMNUser", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncUsgMNUser", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQUsgMNUser
		rownode = getRownode(srcdoc, "ListFmncQUsgMNUser", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncUsgMNUser", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncUsgMNUser", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncUsgMNUser", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgMNUser", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgMNUser", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncUsgMNUser", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncUsgMNUser", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncUsgMNUser", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncUsgMNUserData", scrJref, "StgIacQryFmncUsgMNUser");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncUsgMNUser", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncUsgMNUser", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncUsgMNUser", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncUsgMNUserDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "ContInfFmncUsgMNUser", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "StatAppFmncUsgMNUser", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "StatShrFmncUsgMNUser", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "StgIacFmncUsgMNUser", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "TagFmncUsgMNUser", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "ListFmncQUsgMNUser", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "StatAppQryFmncUsgMNUser", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "StatShrQryFmncUsgMNUser", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncUsgMNUserData", "StgIacQryFmncUsgMNUser", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncUsgMNUserData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncUsgMNUserData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncUsgMNUserData") {
