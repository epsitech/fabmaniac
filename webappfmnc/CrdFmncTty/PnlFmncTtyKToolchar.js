/**
  * \file PnlFmncTtyKToolchar.js
  * web client functionality for panel PnlFmncTtyKToolchar
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTtyKToolchar", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTtyKToolchar", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("KToolchar", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("KToolchar_side").setAttribute("height", "30");
	doc.getElementById("KToolchar_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("KToolchar_side").src = "./PnlFmncTtyKToolchar_aside.html";
	doc.getElementById("KToolchar_cont").src = "./PnlFmncTtyKToolchar_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTtyKToolchar", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTtyKToolchar", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("KToolchar", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("KToolchar_side").setAttribute("height", "255");
	doc.getElementById("KToolchar_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("KToolchar_side").src = "./PnlFmncTtyKToolchar_bside.html";
	doc.getElementById("KToolchar_cont").src = "./PnlFmncTtyKToolchar_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KToolchar_side").contentDocument;
	contdoc = doc.getElementById("KToolchar_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("KToolchar_side").contentDocument;
	contdoc = doc.getElementById("KToolchar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KToolchar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KToolchar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KToolchar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTtyKToolchar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTtyKToolchar", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTtyKToolchar", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTtyKToolchar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTtyKToolchar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyKToolchar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKToolchar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKToolchar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKToolchar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKToolchar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKToolchar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKToolchar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKToolchar", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKToolchar", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKToolchar", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKToolchar", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKToolchar", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKToolchar", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKToolchar", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTtyKToolchar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTtyKToolchar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTtyKToolchar
		rownode = getRownode(srcdoc, "ListFmncQTtyKToolchar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTtyKToolchar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTtyKToolchar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTtyKToolchar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KToolchar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KToolchar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KToolchar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KToolchar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KToolchar_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyKToolchar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKToolchar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKToolchar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTtyKToolchar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTtyKToolchar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTtyKToolcharData", scrJref, "StgIacQryFmncTtyKToolchar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKToolchar", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTtyKToolchar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyKToolchar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTtyKToolcharDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "ContInfFmncTtyKToolchar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "StatAppFmncTtyKToolchar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "StatShrFmncTtyKToolchar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "StgIacFmncTtyKToolchar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "TagFmncTtyKToolchar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "ListFmncQTtyKToolchar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "StatAppQryFmncTtyKToolchar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "StatShrQryFmncTtyKToolchar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTtyKToolcharData", "StgIacQryFmncTtyKToolchar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTtyKToolcharData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTtyKToolcharData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KToolchar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTtyKToolcharData") {
