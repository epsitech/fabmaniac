/**
  * \file PnlFmncTtyKSteppar.js
  * web client functionality for panel PnlFmncTtyKSteppar
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTtyKSteppar", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncTtyKSteppar", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("KSteppar", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("KSteppar_side").setAttribute("height", "30");
	doc.getElementById("KSteppar_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("KSteppar_side").src = "./PnlFmncTtyKSteppar_aside.html";
	doc.getElementById("KSteppar_cont").src = "./PnlFmncTtyKSteppar_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncTtyKSteppar", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncTtyKSteppar", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("KSteppar", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("KSteppar_side").setAttribute("height", "255");
	doc.getElementById("KSteppar_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("KSteppar_side").src = "./PnlFmncTtyKSteppar_bside.html";
	doc.getElementById("KSteppar_cont").src = "./PnlFmncTtyKSteppar_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KSteppar_side").contentDocument;
	contdoc = doc.getElementById("KSteppar_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("KSteppar_side").contentDocument;
	contdoc = doc.getElementById("KSteppar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KSteppar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KSteppar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KSteppar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTtyKSteppar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTtyKSteppar", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncTtyKSteppar", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTtyKSteppar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTtyKSteppar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyKSteppar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKSteppar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKSteppar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKSteppar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKSteppar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKSteppar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKSteppar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKSteppar", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKSteppar", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKSteppar", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKSteppar", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKSteppar", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKSteppar", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyKSteppar", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncTtyKSteppar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTtyKSteppar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTtyKSteppar
		rownode = getRownode(srcdoc, "ListFmncQTtyKSteppar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTtyKSteppar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTtyKSteppar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTtyKSteppar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KSteppar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KSteppar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KSteppar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KSteppar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KSteppar_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyKSteppar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKSteppar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyKSteppar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTtyKSteppar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTtyKSteppar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTtyKStepparData", scrJref, "StgIacQryFmncTtyKSteppar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyKSteppar", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTtyKSteppar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyKSteppar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTtyKStepparDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "ContInfFmncTtyKSteppar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "StatAppFmncTtyKSteppar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "StatShrFmncTtyKSteppar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "StgIacFmncTtyKSteppar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "TagFmncTtyKSteppar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "ListFmncQTtyKSteppar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "StatAppQryFmncTtyKSteppar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "StatShrQryFmncTtyKSteppar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTtyKStepparData", "StgIacQryFmncTtyKSteppar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncTtyKStepparData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTtyKStepparData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KSteppar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTtyKStepparData") {
