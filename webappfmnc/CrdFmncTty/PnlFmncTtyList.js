/**
  * \file PnlFmncTtyList.js
  * web client functionality for panel PnlFmncTtyList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncTtyList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncTtyList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncTtyList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncTtyList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTtyList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncTtyList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncTtyList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncTtyList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncTtyList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyList", "TcoTitWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncTtyList", "TcoSupWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncTtyList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncTtyList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncTtyList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQTtyList
		rownode = getRownode(srcdoc, "ListFmncQTtyList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncTtyList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncTtyList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncTtyList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncTtyList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncTtyList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncTtyList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTtyListData", scrJref, "StgIacQryFmncTtyList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncTtyList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncTtyList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncTtyList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncTtyListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncTtyList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncTtyList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncTtyListData", scrJref, "ContIacFmncTtyList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "ContIacFmncTtyList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "ContInfFmncTtyList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "StatShrFmncTtyList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "StgIacFmncTtyList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "TagFmncTtyList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "ListFmncQTtyList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "StatAppQryFmncTtyList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "StatShrQryFmncTtyList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncTtyListData", "StgIacQryFmncTtyList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncTtyListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncTtyListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncTtyListData") {
				mask = mergeDpchEngData(dom);
