/**
  * \file CrdFmncFab.js
  * web client functionality for card CrdFmncFab
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppFmncFab", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppFmncFab", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppFmncFab", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppFmncFab", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppFmncFab", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlFmncFabHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlFmncFabList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlFmncFabRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppFmncFab", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppFmncFab", "shortMenu", short);
	setSi(srcdoc, "StatAppFmncFab", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppFmncFab", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncFab", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagFmncFab", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagFmncFab", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrFmncFab", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrFmncFab", "MitCrdNewAvail") == "true");
	MitCrdImfAvail = (retrieveSi(srcdoc, "StatShrFmncFab", "MitCrdImfAvail") == "true");
	MitCrdExfActive = (retrieveSi(srcdoc, "StatShrFmncFab", "MitCrdExfActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppFmncFab", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdFab"), retrieveCi(srcdoc, "ContInfFmncFab", "MtxCrdFab"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagFmncFab", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdImf"), retrieveTi(srcdoc, "TagFmncFab", "MitCrdImf"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdExf"), retrieveTi(srcdoc, "TagFmncFab", "MitCrdExf"));
	setMitActive("MitCrdExf", MitCrdExfActive);

	height -= setMitMspAvail("MspCrd1", MspCrd1Avail, 1);
	height -= setMitMspAvail("MitCrdNew", MitCrdNewAvail, 20);
	height -= setMitMspAvail("MitCrdImf", MitCrdImfAvail, 20);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgFmncFabImpfab") || (sref == "DlgFmncFabExpfab")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgexpfab = retrieveSi(srcdoc, "StatShrFmncFab", "scrJrefDlgexpfab");
	var scrJrefDlgimpfab = retrieveSi(srcdoc, "StatShrFmncFab", "scrJrefDlgimpfab");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrFmncFab", "scrJrefDlgnew");

	if (scrJrefDlgexpfab != "") {
		if (scrJrefDlg != scrJrefDlgexpfab) showDlg("DlgFmncFabExpfab", scrJrefDlgexpfab);
	} else if (scrJrefDlgimpfab != "") {
		if (scrJrefDlg != scrJrefDlgimpfab) showDlg("DlgFmncFabImpfab", scrJrefDlgimpfab);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgFmncFabNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfFmncFab", "MtxCrdFab") + " - FabManiac 0.1.40";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFabDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFabDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfFmncFab", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncFabData", "ContInfFmncFab", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncFabData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngFmncFabData", "StatAppFmncFab", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncFabData", "StatShrFmncFab", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncFabData", "TagFmncFab", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncFabData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrFmncFab", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrFmncFab", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrFmncFab", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncFabData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncFab", "srefIxFmncVReqitmode");

	if (srefIxFmncVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxFmncVReqitmode = "notify";
		setSi(srcdoc, "StatAppFmncFab", "srefIxFmncVReqitmode", srefIxFmncVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxFmncVReqitmode = retrieveSi(srcdoc, "StatAppFmncFab", "srefIxFmncVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppFmncFab", "latency"));
