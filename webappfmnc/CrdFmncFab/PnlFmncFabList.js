/**
  * \file PnlFmncFabList.js
  * web client functionality for panel PnlFmncFabList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncFabList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFabList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncFabList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncFabList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFabList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncFabList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncFabList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncFabList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncFabList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFabList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFabList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFabList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFabList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFabList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFabList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFabList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFabList", "ncol"));

		var TcoOrgWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncFabList", "TcoOrgWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFabList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncFabList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncFabList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQFabList
		rownode = getRownode(srcdoc, "ListFmncQFabList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncFabList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncFabList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncFabList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFabList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFabList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncFabList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncFabList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncFabList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFabListData", scrJref, "StgIacQryFmncFabList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncFabList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncFabList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncFabList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncFabListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncFabList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncFabList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncFabListData", scrJref, "ContIacFmncFabList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "ContIacFmncFabList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "ContInfFmncFabList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "StatShrFmncFabList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "StgIacFmncFabList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "TagFmncFabList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "ListFmncQFabList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "StatAppQryFmncFabList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "StatShrQryFmncFabList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncFabListData", "StgIacQryFmncFabList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncFabListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncFabListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncFabListData") {
				mask = mergeDpchEngData(dom);
