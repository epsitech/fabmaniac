/**
  * \file PnlFmncStpList.js
  * web client functionality for panel PnlFmncStpList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlFmncStpList_aside.html";
	doc.getElementById("List_cont").src = "./PnlFmncStpList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlFmncStpList_bside.html";
	doc.getElementById("List_cont").src = "./PnlFmncStpList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncStpList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncStpList", "Trs"));
...
function init() {
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatShrFmncStpList", "srefIxFmncVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncStpList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncStpList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncStpList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncStpList", "TcoTitWidth"));
		var TcoActWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncStpList", "TcoActWidth"));
		var TcoRltWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncStpList", "TcoRltWidth"));
		var TcoRluWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncStpList", "TcoRluWidth"));
		var TcoStpWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncStpList", "TcoStpWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncStpList", "TcoStaWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncStpList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagFmncStpList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncStpList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQStpList
		rownode = getRownode(srcdoc, "ListFmncQStpList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncStpList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncStpList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncStpList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncStpList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncStpList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncStpList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncStpListData", scrJref, "StgIacQryFmncStpList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpList", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncStpList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncStpList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncStpListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacFmncStpList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacFmncStpList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncStpListData", scrJref, "ContIacFmncStpList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "ContIacFmncStpList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "ContInfFmncStpList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "StatShrFmncStpList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "StgIacFmncStpList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "TagFmncStpList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "ListFmncQStpList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "StatAppQryFmncStpList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "StatShrQryFmncStpList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncStpListData", "StgIacQryFmncStpList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngFmncStpListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncStpListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncStpListData") {
				mask = mergeDpchEngData(dom);
