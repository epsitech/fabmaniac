/**
  * \file PnlFmncStpTpl1NStep.js
  * web client functionality for panel PnlFmncStpTpl1NStep
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncStpTpl1NStep", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncStpTpl1NStep", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("Tpl1NStep", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Tpl1NStep_side").setAttribute("height", "30");
	doc.getElementById("Tpl1NStep_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Tpl1NStep_side").src = "./PnlFmncStpTpl1NStep_aside.html";
	doc.getElementById("Tpl1NStep_cont").src = "./PnlFmncStpTpl1NStep_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncStpTpl1NStep", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncStpTpl1NStep", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("Tpl1NStep", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Tpl1NStep_side").setAttribute("height", "255");
	doc.getElementById("Tpl1NStep_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Tpl1NStep_side").src = "./PnlFmncStpTpl1NStep_bside.html";
	doc.getElementById("Tpl1NStep_cont").src = "./PnlFmncStpTpl1NStep_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Tpl1NStep_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NStep_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("Tpl1NStep_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NStep_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Tpl1NStep_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Tpl1NStep_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Tpl1NStep_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncStpTpl1NStep", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncStpTpl1NStep", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncStpTpl1NStep", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncStpTpl1NStep", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncStpTpl1NStep", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncStpTpl1NStep", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpTpl1NStep", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpTpl1NStep", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpTpl1NStep", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncStpTpl1NStep", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncStpTpl1NStep", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQStpTpl1NStep
		rownode = getRownode(srcdoc, "ListFmncQStpTpl1NStep", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncStpTpl1NStep", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncStpTpl1NStep", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncStpTpl1NStep", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Tpl1NStep", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NStep_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NStep_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Tpl1NStep_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Tpl1NStep_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncStpTpl1NStep", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncStpTpl1NStep", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncStpTpl1NStep", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncStpTpl1NStep", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncStpTpl1NStepData", scrJref, "StgIacQryFmncStpTpl1NStep");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncStpTpl1NStep", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncStpTpl1NStep", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncStpTpl1NStepDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "ContInfFmncStpTpl1NStep", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "StatAppFmncStpTpl1NStep", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "StatShrFmncStpTpl1NStep", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "StgIacFmncStpTpl1NStep", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "TagFmncStpTpl1NStep", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "ListFmncQStpTpl1NStep", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "StatAppQryFmncStpTpl1NStep", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "StatShrQryFmncStpTpl1NStep", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncStpTpl1NStepData", "StgIacQryFmncStpTpl1NStep", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncStpTpl1NStepData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncStpTpl1NStepData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Tpl1NStep");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncStpTpl1NStepData") {
