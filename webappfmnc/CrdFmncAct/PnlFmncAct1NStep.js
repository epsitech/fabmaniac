/**
  * \file PnlFmncAct1NStep.js
  * web client functionality for panel PnlFmncAct1NStep
...
// IP minimize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncAct1NStep", "srefIxFmncVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppFmncAct1NStep", "srefIxFmncVExpstate", "mind");

// IP minimize.estapp --- END
	getRecwnd().changeHeight("1NStep", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NStep_side").setAttribute("height", "30");
	doc.getElementById("1NStep_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NStep_side").src = "./PnlFmncAct1NStep_aside.html";
	doc.getElementById("1NStep_cont").src = "./PnlFmncAct1NStep_a.html";
};

function regularize() {
// IP regularize.estapp --- BEGIN
	if (retrieveSi(srcdoc, "StatAppFmncAct1NStep", "srefIxFmncVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppFmncAct1NStep", "srefIxFmncVExpstate", "regd");

// IP regularize.estapp --- END
	getRecwnd().changeHeight("1NStep", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NStep_side").setAttribute("height", "255");
	doc.getElementById("1NStep_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NStep_side").src = "./PnlFmncAct1NStep_bside.html";
	doc.getElementById("1NStep_cont").src = "./PnlFmncAct1NStep_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NStep_side").contentDocument;
	contdoc = doc.getElementById("1NStep_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initB() {
	...
	sidedoc = doc.getElementById("1NStep_side").contentDocument;
	contdoc = doc.getElementById("1NStep_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NStep_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NStep_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NStep_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncAct1NStep", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagFmncAct1NStep", "Trs"));
...
// IP init.estapp --- BEGIN
	var srefIxFmncVExpstate = retrieveSi(srcdoc, "StatAppFmncAct1NStep", "srefIxFmncVExpstate");
// IP init.estapp --- END
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagFmncAct1NStep", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryFmncAct1NStep", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryFmncAct1NStep", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAct1NStep", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAct1NStep", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAct1NStep", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAct1NStep", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAct1NStep", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAct1NStep", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAct1NStep", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacFmncAct1NStep", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagFmncAct1NStep", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryFmncAct1NStep", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListFmncQAct1NStep
		rownode = getRownode(srcdoc, "ListFmncQAct1NStep", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagFmncAct1NStep", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagFmncAct1NStep", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfFmncAct1NStep", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NStep", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NStep_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NStep_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NStep_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NStep_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncAct1NStep", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAct1NStep", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryFmncAct1NStep", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryFmncAct1NStep", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryFmncAct1NStep", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppFmncAct1NStepData", scrJref, "StgIacQryFmncAct1NStep");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryFmncAct1NStep", "firstcol"));

	setSi(srcdoc, "StatAppQryFmncAct1NStep", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryFmncAct1NStep", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppFmncAct1NStepDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "ContInfFmncAct1NStep", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "StatAppFmncAct1NStep", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "StatShrFmncAct1NStep", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "StgIacFmncAct1NStep", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "TagFmncAct1NStep", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "ListFmncQAct1NStep", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "StatAppQryFmncAct1NStep", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "StatShrQryFmncAct1NStep", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngFmncAct1NStepData", "StgIacQryFmncAct1NStep", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngFmncAct1NStepData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngFmncAct1NStepData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NStep");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngFmncAct1NStepData") {
