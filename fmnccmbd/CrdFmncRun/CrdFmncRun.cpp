/**
  * \file CrdFmncRun.cpp
  * job handler for job CrdFmncRun (implementation)
...
#include "CrdFmncRun_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "CrdFmncRun_evals.cpp"
// IP evalsInclude --- END
...
CrdFmncRun::CrdFmncRun(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
			, const ubigint ref
		) : JobFmnc(xchg, VecFmncVJob::CRDFMNCRUN, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlFmncRunRec(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnlheadbar = new PnlFmncRunHeadbar(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnllist = new PnlFmncRunList(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP constructor.cust2 --- INSERT
	...
	if ((ref+1) == 0) {
		dlgnew = new DlgFmncRunNew(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* CrdFmncRun::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdFmncRun/" + VecFmncVLocale::getSref(ixFmncVLocale);
	continf.MtxCrdRun = StubFmnc::getStubRunStd(dbsfmnc, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFRUN, jref), ixFmncVLocale, Stub::VecVNonetype::FULL);
...
void CrdFmncRun::changeRef(
	...
	uint ixFmncVAccess;
	xchg->triggerIxRefToIxCall(dbsfmnc, VecFmncVCall::CALLFMNCACCESS, jref, VecFmncVMaintable::TBLFMNCMRUN, ref, ixFmncVAccess);

	if (ixFmncVAccess != VecFmncVAccess::NONE) {
		xchg->addIxPreset(VecFmncVPreset::PREFMNCIXACC, jref, ixFmncVAccess);

		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsfmnc, VecFmncVCall::CALLFMNCLOG, jref, VecFmncVPreset::VOID, 0, "CrdFmncRun", ref);
		xchg->addRefPreset(VecFmncVPreset::PREFMNCREFRUN, jref, ref);

		refresh(dbsfmnc, moditems);
		if (notif) xchg->submitDpch(getNewDpchEng(moditems));

		if (pnllist) pnllist->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFRUN, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFRUN, jrefTrig, notif);
	};
};

void CrdFmncRun::updatePreset(
			DbsFmnc* dbsfmnc
...
void CrdFmncRun::leaveSgeIdle(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdFmncRun::enterSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
...
void CrdFmncRun::leaveSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeAlrfmncabt --- INSERT
};

uint CrdFmncRun::enterSgeClose(
			DbsFmnc* dbsfmnc
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDCLOSE, jref, VecFmncVCard::CRDFMNCRUN);
	return retval;
};

void CrdFmncRun::leaveSgeClose(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeClose --- INSERT
};

string CrdFmncRun::getSquawk(
			DbsFmnc* dbsfmnc
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdFmncRun::handleRequest(
	...
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCRUNDO) {
...
void CrdFmncRun::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void CrdFmncRun::handleDpchAppDoClose(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	muteRefresh = true;
...
void CrdFmncRun::handleDpchAppDoMitAppAbtClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void CrdFmncRun::handleDpchAppDoMitCrdNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	if (!dlgnew) {
...
void CrdFmncRun::handleDpchAppDoMitCrdGnrClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	if (!dlggenrep) {
		dlggenrep = new DlgFmncRunGenrep(xchg, dbsfmnc, jref, ixFmncVLocale);
...
void CrdFmncRun::handleDpchAppFmncAlert(
			DbsFmnc* dbsfmnc
...
bool CrdFmncRun::handleCallFmncDlgClose(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncRun::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncRun::handleCallFmncRefPreSet(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecFmncVPreset::PREFMNCREFRUN) {
