/**
  * \file PnlFmncRunMNPerson.cpp
  * job handler for job PnlFmncRunMNPerson (implementation)
...
#include "PnlFmncRunMNPerson_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncRunMNPerson_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncRunMNPerson
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncRunMNPerson::PnlFmncRunMNPerson(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCRUNMNPERSON, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncRunMNPerson(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncRunMNPerson::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncRunMNPerson::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncRunMNPerson::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncRunMNPerson::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCRUNMNPERSONDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCRUNMNPERSONDO) {
...
void PnlFmncRunMNPerson::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncRunMNPerson::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncRunMNPerson::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncRunMNPerson::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncRunMNPerson::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncRunMNPerson::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncRunMNPerson::StgIac::JNUM)) {
...
void PnlFmncRunMNPerson::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQRunMNPerson WHERE ref = " + to_string(refSel), refSel)) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncPrs", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngFmncConfirm(false, 0, "");
	else *dpcheng = new DpchEngFmncConfirm(true, jrefNew, "CrdFmncPrs");
};

void PnlFmncRunMNPerson::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncRunMNPerson::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncRunMNPerson::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncRunMNPerson::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncRunMNPerson::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
