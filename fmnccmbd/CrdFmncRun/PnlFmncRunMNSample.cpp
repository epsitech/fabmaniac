/**
  * \file PnlFmncRunMNSample.cpp
  * job handler for job PnlFmncRunMNSample (implementation)
...
#include "PnlFmncRunMNSample_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncRunMNSample_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncRunMNSample
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncRunMNSample::PnlFmncRunMNSample(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCRUNMNSAMPLE, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncRunMNSample(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncRunMNSample::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncRunMNSample::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncRunMNSample::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncRunMNSample::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCRUNMNSAMPLEDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCRUNMNSAMPLEDO) {
...
void PnlFmncRunMNSample::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncRunMNSample::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncRunMNSample::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncRunMNSample::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncRunMNSample::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncRunMNSample::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncRunMNSample::StgIac::JNUM)) {
...
void PnlFmncRunMNSample::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQRunMNSample WHERE ref = " + to_string(refSel), refSel)) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncSmp", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngFmncConfirm(false, 0, "");
	else *dpcheng = new DpchEngFmncConfirm(true, jrefNew, "CrdFmncSmp");
};

void PnlFmncRunMNSample::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncRunMNSample::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncRunMNSample::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncRunMNSample::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncRunMNSample::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
