/**
  * \file PnlFmncTtyMNTool_blks.cpp
  * job handler for job PnlFmncTtyMNTool (implementation of blocks)
...
void PnlFmncTtyMNTool::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncTtyMNTool";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncTtyMNTool";
...
void PnlFmncTtyMNTool::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncTtyMNTool";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncTtyMNTool";
...
void PnlFmncTtyMNTool::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncTtyMNTool";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncTtyMNTool";
...
void PnlFmncTtyMNTool::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncTtyMNTool";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncTtyMNTool";
...
PnlFmncTtyMNTool::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTTYMNTOOLDATA) {
};
...
void PnlFmncTtyMNTool::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTtyMNToolData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTtyMNTool::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTTYMNTOOLDO) {
	ixVDo = 0;
};
...
void PnlFmncTtyMNTool::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTtyMNToolDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTtyMNTool::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryFmncTtyMNTool::StatShr* statshrqry
			, QryFmncTtyMNTool::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCTTYMNTOOLDATA, jref) {
...
void PnlFmncTtyMNTool::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncTtyMNTool::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncTtyMNToolData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncTtyMNTool::StatApp::writeXML(wr);
