/**
  * \file PnlFmncTtyKSteppar_blks.cpp
  * job handler for job PnlFmncTtyKSteppar (implementation of blocks)
...
void PnlFmncTtyKSteppar::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncTtyKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncTtyKSteppar";
...
void PnlFmncTtyKSteppar::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncTtyKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncTtyKSteppar";
...
void PnlFmncTtyKSteppar::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncTtyKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncTtyKSteppar";
...
void PnlFmncTtyKSteppar::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncTtyKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncTtyKSteppar";
...
PnlFmncTtyKSteppar::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTTYKSTEPPARDATA) {
};
...
void PnlFmncTtyKSteppar::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTtyKStepparData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTtyKSteppar::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTTYKSTEPPARDO) {
	ixVDo = 0;
};
...
void PnlFmncTtyKSteppar::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTtyKStepparDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTtyKSteppar::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryFmncTtyKSteppar::StatShr* statshrqry
			, QryFmncTtyKSteppar::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCTTYKSTEPPARDATA, jref) {
...
void PnlFmncTtyKSteppar::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncTtyKSteppar::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncTtyKStepparData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncTtyKSteppar::StatApp::writeXML(wr);
