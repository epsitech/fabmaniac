/**
  * \file DlgFmncFabNew.cpp
  * job handler for job DlgFmncFabNew (implementation)
...
#include "DlgFmncFabNew_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "DlgFmncFabNew_evals.cpp"
// IP evalsInclude --- END
...
DlgFmncFabNew::DlgFmncFabNew(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::DLGFMNCFABNEW, jrefSup, ixFmncVLocale) {
...
// IP constructor.addJob --- END
	...
	feedFDetPupOrg.tag = "FeedFDetPupOrg";
...
// IP cust --- INSERT
DpchEngFmnc* DlgFmncFabNew::getNewDpchEng(
			set<uint> items
		) {
	...
	if (items.empty()) {
		...
		dpcheng = new DpchEngData(jref, &contiac, &continf, &feedFDetPupOrg, &feedFSge, &statshr, items);
	};

	return dpcheng;
};

void DlgFmncFabNew::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void DlgFmncFabNew::changeStage(
			DbsFmnc* dbsfmnc
			, uint _ixVSge
			, DpchEngFmnc** dpcheng
		) {
...
uint DlgFmncFabNew::enterSgeIdle(
			DbsFmnc* dbsfmnc
		) {
...
void DlgFmncFabNew::leaveSgeIdle(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeIdle --- INSERT
};

uint DlgFmncFabNew::enterSgeCreate(
			DbsFmnc* dbsfmnc
		) {
...
void DlgFmncFabNew::leaveSgeCreate(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeCreate --- INSERT
};

uint DlgFmncFabNew::enterSgeDone(
			DbsFmnc* dbsfmnc
		) {
...
void DlgFmncFabNew::leaveSgeDone(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeDone --- INSERT
};

string DlgFmncFabNew::getSquawk(
			DbsFmnc* dbsfmnc
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void DlgFmncFabNew::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPDLGFMNCFABNEWDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPDLGFMNCFABNEWDO) {
...
void DlgFmncFabNew::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void DlgFmncFabNew::handleDpchAppDataContiac(
			DbsFmnc* dbsfmnc
			, ContIac* _contiac
			, DpchEngFmnc** dpcheng
		) {
...
void DlgFmncFabNew::handleDpchAppDoButCncClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void DlgFmncFabNew::handleDpchAppDoButCreClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
