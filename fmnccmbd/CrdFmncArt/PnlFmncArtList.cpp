/**
  * \file PnlFmncArtList.cpp
  * job handler for job PnlFmncArtList (implementation)
...
#include "PnlFmncArtList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncArtList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncArtList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncArtList::PnlFmncArtList(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCARTLIST, jrefSup, ixFmncVLocale) {
...
// IP constructor.addJob --- END
	...
	QryFmncArtList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecFmncVPreset::PREFMNCREFSEL, jref, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFART, jref));
	xchg->addIxPreset(VecFmncVPreset::PREFMNCIXORD, jref, QryFmncArtList::VecVOrd::SRF);

	qry = new QryFmncArtList(xchg, dbsfmnc, jref, ixFmncVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixFmncVExpstate = ((xchg->getRefPreset(VecFmncVPreset::PREFMNCREFART, jref)) ? VecFmncVExpstate::MIND : VecFmncVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncArtList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncArtList::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncArtList::regularize(
			DbsFmnc* dbsfmnc
			, const bool notif
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArtList::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCARTLISTDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCARTLISTDO) {
...
void PnlFmncArtList::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncArtList::handleDpchAppDataContiac(
			DbsFmnc* dbsfmnc
			, ContIac* _contiac
			, DpchEngFmnc** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryFmncArtList::VecVOrd::SRF) && (_contiac->numFTos <= QryFmncArtList::VecVOrd::TOL)) {
...
void PnlFmncArtList::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArtList::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncArtList::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncArtList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncArtList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			refresh(dbsfmnc, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncArtList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsfmnc, VecFmncVCall::CALLFMNCREFPRESET, jref, VecFmncVPreset::PREFMNCREFART, refSelNew);
		};
	};
...
void PnlFmncArtList::handleDpchAppDoButMinimizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArtList::handleDpchAppDoButRegularizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArtList::handleDpchAppDoButNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArtList::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArtList::handleDpchAppDoButFilterClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArtList::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncArtList::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncArtList::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
