/**
  * \file PnlFmncArtRec_blks.cpp
  * job handler for job PnlFmncArtRec (implementation of blocks)
...
void PnlFmncArtRec::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncArtRec";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncArtRec";
...
void PnlFmncArtRec::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppFmncArtRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncArtRec";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeBoolAttr(wr, itemtag, "sref", "initdoneDetail", initdoneDetail);
		writeBoolAttr(wr, itemtag, "sref", "initdone1NSample", initdone1NSample);
...
PnlFmncArtRec::StatShr::StatShr(
	...
	this->jref1NSample = jref1NSample;
	this->jref1NStockitem = jref1NStockitem;
	this->jrefRef1NFile = jrefRef1NFile;
	this->jrefMNOrg = jrefMNOrg;
	this->ButRegularizeActive = ButRegularizeActive;

	mask = {IXFMNCVEXPSTATE, JREFDETAIL, JREF1NSAMPLE, JREF1NSTOCKITEM, JREFREF1NFILE, JREFMNORG, BUTREGULARIZEACTIVE};
};

void PnlFmncArtRec::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrFmncArtRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncArtRec";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		...
		writeStringAttr(wr, itemtag, "sref", "scrJref1NSample", Scr::scramble(mScr, scr, descr, jref1NSample));
...
set<uint> PnlFmncArtRec::StatShr::comm(
	...
	if (jref1NSample == comp->jref1NSample) insert(items, JREF1NSAMPLE);
...
set<uint> PnlFmncArtRec::StatShr::diff(
	...
	diffitems = {IXFMNCVEXPSTATE, JREFDETAIL, JREF1NSAMPLE, JREF1NSTOCKITEM, JREFREF1NFILE, JREFMNORG, BUTREGULARIZEACTIVE};
	for (auto it=commitems.begin();it!=commitems.end();it++) diffitems.erase(*it);
...
void PnlFmncArtRec::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncArtRec";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncArtRec";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Article");
...
PnlFmncArtRec::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCARTRECDO) {
	ixVDo = 0;
};
...
void PnlFmncArtRec::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncArtRecDo");
	else
...
PnlFmncArtRec::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCARTRECDATA, jref) {
...
void PnlFmncArtRec::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncArtRec::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncArtRecData");
