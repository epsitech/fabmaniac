/**
  * \file PnlFmncArt1NSample.cpp
  * job handler for job PnlFmncArt1NSample (implementation)
...
#include "PnlFmncArt1NSample_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncArt1NSample_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncArt1NSample
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncArt1NSample::PnlFmncArt1NSample(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCART1NSAMPLE, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncArt1NSample(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncArt1NSample::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncArt1NSample::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncArt1NSample::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncArt1NSample::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCART1NSAMPLEDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCART1NSAMPLEDO) {
...
void PnlFmncArt1NSample::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncArt1NSample::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArt1NSample::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncArt1NSample::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncArt1NSample::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncArt1NSample::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncArt1NSample::StgIac::JNUM)) {
...
void PnlFmncArt1NSample::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
...
void PnlFmncArt1NSample::handleDpchAppDoButNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlFmncArt1NSample::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncArt1NSample::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncArt1NSample::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncArt1NSample::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
