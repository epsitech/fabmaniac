/**
  * \file PnlFmncFilMNStep.cpp
  * job handler for job PnlFmncFilMNStep (implementation)
...
#include "PnlFmncFilMNStep_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncFilMNStep_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncFilMNStep
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncFilMNStep::PnlFmncFilMNStep(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCFILMNSTEP, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncFilMNStep(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncFilMNStep::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncFilMNStep::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncFilMNStep::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFILMNSTEPDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFILMNSTEPDO) {
...
void PnlFmncFilMNStep::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncFilMNStep::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilMNStep::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncFilMNStep::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncFilMNStep::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncFilMNStep::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncFilMNStep::StgIac::JNUM)) {
...
void PnlFmncFilMNStep::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilMNStep::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilMNStep::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncFilMNStep::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncFilMNStep::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
