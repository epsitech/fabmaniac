/**
  * \file PnlFmncFilDetail_blks.cpp
  * job handler for job PnlFmncFilDetail (implementation of blocks)
...
void PnlFmncFilDetail::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncFilDetail";
...
void PnlFmncFilDetail::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppFmncFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncFilDetail";
...
void PnlFmncFilDetail::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncFilDetail";
...
void PnlFmncFilDetail::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncFilDetail";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "CptClu", "Geb\\u00fcndelt mit");
			...
			writeStringAttr(wr, itemtag, "sref", "CptFnm", "nom de fichier");
			writeStringAttr(wr, itemtag, "sref", "CptClu", "Group\\u00e9 avec");
			writeStringAttr(wr, itemtag, "sref", "CptReu", "r\\u00e9f\\u00e9rence");
			writeStringAttr(wr, itemtag, "sref", "CptCnt", "contenu");
			writeStringAttr(wr, itemtag, "sref", "CptAcv", "temps d'archivage");
			writeStringAttr(wr, itemtag, "sref", "CptAnm", "nom de fichier en archive");
			writeStringAttr(wr, itemtag, "sref", "CptMim", "type MIME");
			writeStringAttr(wr, itemtag, "sref", "CptSiz", "taille [kO]");
...
PnlFmncFilDetail::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFILDETAILDATA) {
};
...
void PnlFmncFilDetail::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFilDetailData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFilDetail::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFILDETAILDO) {
	ixVDo = 0;
};
...
void PnlFmncFilDetail::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFilDetailDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFilDetail::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFLstClu
			...
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCFILDETAILDATA, jref) {
...
void PnlFmncFilDetail::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncFilDetail::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncFilDetailData");
