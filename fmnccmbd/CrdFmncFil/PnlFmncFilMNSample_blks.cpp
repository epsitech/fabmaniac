/**
  * \file PnlFmncFilMNSample_blks.cpp
  * job handler for job PnlFmncFilMNSample (implementation of blocks)
...
void PnlFmncFilMNSample::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncFilMNSample";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncFilMNSample";
...
void PnlFmncFilMNSample::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncFilMNSample";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncFilMNSample";
...
void PnlFmncFilMNSample::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncFilMNSample";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncFilMNSample";
...
void PnlFmncFilMNSample::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncFilMNSample";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncFilMNSample";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Samples used for");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Sample");
		} else if (ixFmncVLocale == VecFmncVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Verwendung f\\u00fcr Samples");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Sample");
		} else if (ixFmncVLocale == VecFmncVLocale::FRCH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Utilisation pour \\u00e9chantillons");
...
PnlFmncFilMNSample::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFILMNSAMPLEDATA) {
};
...
void PnlFmncFilMNSample::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFilMNSampleData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFilMNSample::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFILMNSAMPLEDO) {
	ixVDo = 0;
};
...
void PnlFmncFilMNSample::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFilMNSampleDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFilMNSample::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryFmncFilMNSample::StatShr* statshrqry
			, QryFmncFilMNSample::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCFILMNSAMPLEDATA, jref) {
...
void PnlFmncFilMNSample::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncFilMNSample::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncFilMNSampleData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncFilMNSample::StatApp::writeXML(wr);
