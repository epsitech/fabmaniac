/**
  * \file PnlFmncFilList.cpp
  * job handler for job PnlFmncFilList (implementation)
...
#include "PnlFmncFilList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncFilList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncFilList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncFilList::PnlFmncFilList(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCFILLIST, jrefSup, ixFmncVLocale) {
...
// IP constructor.addJob --- END
	...
	QryFmncFilList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecFmncVPreset::PREFMNCREFSEL, jref, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFFIL, jref));
	xchg->addIxPreset(VecFmncVPreset::PREFMNCIXORD, jref, QryFmncFilList::VecVOrd::FNM);

	qry = new QryFmncFilList(xchg, dbsfmnc, jref, ixFmncVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixFmncVExpstate = ((xchg->getRefPreset(VecFmncVPreset::PREFMNCREFFIL, jref)) ? VecFmncVExpstate::MIND : VecFmncVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncFilList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncFilList::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncFilList::regularize(
			DbsFmnc* dbsfmnc
			, const bool notif
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilList::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFILLISTDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFILLISTDO) {
...
void PnlFmncFilList::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncFilList::handleDpchAppDataContiac(
			DbsFmnc* dbsfmnc
			, ContIac* _contiac
			, DpchEngFmnc** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryFmncFilList::VecVOrd::FNM) && (_contiac->numFTos <= QryFmncFilList::VecVOrd::REU)) {
...
void PnlFmncFilList::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilList::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncFilList::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncFilList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncFilList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			refresh(dbsfmnc, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncFilList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsfmnc, VecFmncVCall::CALLFMNCREFPRESET, jref, VecFmncVPreset::PREFMNCREFFIL, refSelNew);
		};
	};
...
void PnlFmncFilList::handleDpchAppDoButMinimizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilList::handleDpchAppDoButRegularizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilList::handleDpchAppDoButNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilList::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilList::handleDpchAppDoButFilterClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFilList::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncFilList::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncFilList::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
