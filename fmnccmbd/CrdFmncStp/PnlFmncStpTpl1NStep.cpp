/**
  * \file PnlFmncStpTpl1NStep.cpp
  * job handler for job PnlFmncStpTpl1NStep (implementation)
...
#include "PnlFmncStpTpl1NStep_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncStpTpl1NStep_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncStpTpl1NStep
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncStpTpl1NStep::PnlFmncStpTpl1NStep(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCSTPTPL1NSTEP, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncStpTpl1NStep(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncStpTpl1NStep::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncStpTpl1NStep::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncStpTpl1NStep::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncStpTpl1NStep::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSTPTPL1NSTEPDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSTPTPL1NSTEPDO) {
...
void PnlFmncStpTpl1NStep::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncStpTpl1NStep::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncStpTpl1NStep::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncStpTpl1NStep::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncStpTpl1NStep::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncStpTpl1NStep::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncStpTpl1NStep::StgIac::JNUM)) {
...
void PnlFmncStpTpl1NStep::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncStpTpl1NStep::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncStpTpl1NStep::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncStpTpl1NStep::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
