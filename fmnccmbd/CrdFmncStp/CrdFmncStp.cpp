/**
  * \file CrdFmncStp.cpp
  * job handler for job CrdFmncStp (implementation)
...
CrdFmncStp::CrdFmncStp(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
			, const ubigint ref
		) : JobFmnc(xchg, VecFmncVJob::CRDFMNCSTP, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlFmncStpRec(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnlheadbar = new PnlFmncStpHeadbar(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnllist = new PnlFmncStpList(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* CrdFmncStp::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdFmncStp/" + VecFmncVLocale::getSref(ixFmncVLocale);
	continf.MtxCrdStp = StubFmnc::getStubSteStd(dbsfmnc, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFSTP, jref), ixFmncVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdFmncStp::changeRef(
			DbsFmnc* dbsfmnc
	...
	if (ixFmncVAccess != VecFmncVAccess::NONE) {
		...
		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsfmnc, VecFmncVCall::CALLFMNCLOG, jref, VecFmncVPreset::VOID, 0, "CrdFmncStp", ref);
		xchg->addRefPreset(VecFmncVPreset::PREFMNCREFSTP, jref, ref);

		refresh(dbsfmnc, moditems);
		if (notif) xchg->submitDpch(getNewDpchEng(moditems));

		if (pnllist) pnllist->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFSTP, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFSTP, jrefTrig, notif);
	};
};

void CrdFmncStp::updatePreset(
			DbsFmnc* dbsfmnc
...
void CrdFmncStp::leaveSgeIdle(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdFmncStp::enterSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
...
void CrdFmncStp::leaveSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeAlrfmncabt --- INSERT
};

uint CrdFmncStp::enterSgeClose(
			DbsFmnc* dbsfmnc
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDCLOSE, jref, VecFmncVCard::CRDFMNCSTP);
	return retval;
};

void CrdFmncStp::leaveSgeClose(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeClose --- INSERT
};

string CrdFmncStp::getSquawk(
			DbsFmnc* dbsfmnc
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdFmncStp::handleRequest(
	...
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSTPDO) {
...
void CrdFmncStp::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void CrdFmncStp::handleDpchAppDoClose(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	muteRefresh = true;
...
void CrdFmncStp::handleDpchAppDoMitAppAbtClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void CrdFmncStp::handleDpchAppFmncAlert(
			DbsFmnc* dbsfmnc
...
bool CrdFmncStp::handleCallFmncDlgClose(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncStp::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncStp::handleCallFmncRefPreSet(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecFmncVPreset::PREFMNCREFSTP) {
