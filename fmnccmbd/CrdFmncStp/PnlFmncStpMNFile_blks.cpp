/**
  * \file PnlFmncStpMNFile_blks.cpp
  * job handler for job PnlFmncStpMNFile (implementation of blocks)
...
void PnlFmncStpMNFile::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncStpMNFile";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncStpMNFile";
...
void PnlFmncStpMNFile::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncStpMNFile";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncStpMNFile";
...
void PnlFmncStpMNFile::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncStpMNFile";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncStpMNFile";
...
void PnlFmncStpMNFile::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncStpMNFile";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncStpMNFile";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Datei");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Fichier");
...
PnlFmncStpMNFile::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCSTPMNFILEDATA) {
};
...
void PnlFmncStpMNFile::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncStpMNFileData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncStpMNFile::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCSTPMNFILEDO) {
	ixVDo = 0;
};
...
void PnlFmncStpMNFile::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncStpMNFileDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncStpMNFile::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryFmncStpMNFile::StatShr* statshrqry
			, QryFmncStpMNFile::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCSTPMNFILEDATA, jref) {
...
void PnlFmncStpMNFile::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncStpMNFile::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncStpMNFileData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncStpMNFile::StatApp::writeXML(wr);
