/**
  * \file CrdFmncAdr_blks.cpp
  * job handler for job CrdFmncAdr (implementation of blocks)
...
void CrdFmncAdr::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdFmncAdr::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncAdr";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncAdr";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdAdr", MtxCrdAdr);
...
set<uint> CrdFmncAdr::ContInf::comm(
	...
	if (MtxCrdAdr == comp->MtxCrdAdr) insert(items, MTXCRDADR);
...
void CrdFmncAdr::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppFmncAdr";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncAdr";
...
void CrdFmncAdr::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrFmncAdr";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncAdr";
...
void CrdFmncAdr::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncAdr";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncAdr";
...
CrdFmncAdr::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCADRDO) {
	ixVDo = 0;
};
...
void CrdFmncAdr::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncAdrDo");
	else
...
CrdFmncAdr::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFSge
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCADRDATA, jref) {
...
void CrdFmncAdr::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void CrdFmncAdr::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncAdrData");
