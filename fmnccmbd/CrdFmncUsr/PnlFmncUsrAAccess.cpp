/**
  ...
  * job handler for job PnlFmncUsrAAccess (implementation)
...
#include "PnlFmncUsrAAccess_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncUsrAAccess_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncUsrAAccess
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncUsrAAccess::PnlFmncUsrAAccess(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCUSRAACCESS, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncUsrAAccess(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncUsrAAccess::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncUsrAAccess::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncUsrAAccess::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncUsrAAccess::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCUSRAACCESSDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCUSRAACCESSDO) {
...
void PnlFmncUsrAAccess::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncUsrAAccess::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsrAAccess::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncUsrAAccess::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncUsrAAccess::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncUsrAAccess::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncUsrAAccess::StgIac::JNUM)) {
...
void PnlFmncUsrAAccess::handleDpchAppDoButNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsrAAccess::handleDpchAppDoButDuplicateClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsrAAccess::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsrAAccess::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncUsrAAccess::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncUsrAAccess::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
