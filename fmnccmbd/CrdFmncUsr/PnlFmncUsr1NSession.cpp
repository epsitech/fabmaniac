/**
  ...
  * job handler for job PnlFmncUsr1NSession (implementation)
...
#include "PnlFmncUsr1NSession_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncUsr1NSession_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncUsr1NSession
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncUsr1NSession::PnlFmncUsr1NSession(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCUSR1NSESSION, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncUsr1NSession(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncUsr1NSession::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncUsr1NSession::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncUsr1NSession::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncUsr1NSession::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCUSR1NSESSIONDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCUSR1NSESSIONDO) {
...
void PnlFmncUsr1NSession::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncUsr1NSession::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsr1NSession::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncUsr1NSession::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncUsr1NSession::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncUsr1NSession::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncUsr1NSession::StgIac::JNUM)) {
...
void PnlFmncUsr1NSession::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsr1NSession::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncUsr1NSession::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncUsr1NSession::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
