/**
  * \file PnlFmncUsrMNUsergroup.cpp
  * job handler for job PnlFmncUsrMNUsergroup (implementation)
...
#include "PnlFmncUsrMNUsergroup_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncUsrMNUsergroup_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncUsrMNUsergroup
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncUsrMNUsergroup::PnlFmncUsrMNUsergroup(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCUSRMNUSERGROUP, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncUsrMNUsergroup(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncUsrMNUsergroup::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncUsrMNUsergroup::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncUsrMNUsergroup::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncUsrMNUsergroup::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCUSRMNUSERGROUPDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCUSRMNUSERGROUPDO) {
...
void PnlFmncUsrMNUsergroup::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncUsrMNUsergroup::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsrMNUsergroup::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncUsrMNUsergroup::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncUsrMNUsergroup::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncUsrMNUsergroup::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncUsrMNUsergroup::StgIac::JNUM)) {
...
void PnlFmncUsrMNUsergroup::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQUsrMNUsergroup WHERE ref = " + to_string(refSel), refSel)) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncUsg", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngFmncConfirm(false, 0, "");
	else *dpcheng = new DpchEngFmncConfirm(true, jrefNew, "CrdFmncUsg");
};

void PnlFmncUsrMNUsergroup::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsrMNUsergroup::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncUsrMNUsergroup::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncUsrMNUsergroup::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncUsrMNUsergroup::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
