/**
  ...
  * job handler for job PnlFmncSmpList (implementation)
...
#include "PnlFmncSmpList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncSmpList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncSmpList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncSmpList::PnlFmncSmpList(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCSMPLIST, jrefSup, ixFmncVLocale) {
...
// IP constructor.addJob --- END
	...
	QryFmncSmpList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecFmncVPreset::PREFMNCREFSEL, jref, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFSMP, jref));
	xchg->addIxPreset(VecFmncVPreset::PREFMNCIXORD, jref, QryFmncSmpList::VecVOrd::SRF);

	qry = new QryFmncSmpList(xchg, dbsfmnc, jref, ixFmncVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixFmncVExpstate = ((xchg->getRefPreset(VecFmncVPreset::PREFMNCREFSMP, jref)) ? VecFmncVExpstate::MIND : VecFmncVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncSmpList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncSmpList::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncSmpList::regularize(
			DbsFmnc* dbsfmnc
			, const bool notif
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpList::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSMPLISTDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSMPLISTDO) {
...
void PnlFmncSmpList::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncSmpList::handleDpchAppDataContiac(
			DbsFmnc* dbsfmnc
			, ContIac* _contiac
			, DpchEngFmnc** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryFmncSmpList::VecVOrd::SRF) && (_contiac->numFTos <= QryFmncSmpList::VecVOrd::STE)) {
...
void PnlFmncSmpList::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpList::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncSmpList::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncSmpList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncSmpList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			refresh(dbsfmnc, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncSmpList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsfmnc, VecFmncVCall::CALLFMNCREFPRESET, jref, VecFmncVPreset::PREFMNCREFSMP, refSelNew);
		};
	};
...
void PnlFmncSmpList::handleDpchAppDoButMinimizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpList::handleDpchAppDoButRegularizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpList::handleDpchAppDoButNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpList::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpList::handleDpchAppDoButFilterClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpList::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncSmpList::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncSmpList::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
