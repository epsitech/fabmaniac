/**
  ...
  * job handler for job PnlFmncSmpMNRun (implementation)
...
#include "PnlFmncSmpMNRun_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncSmpMNRun_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncSmpMNRun
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncSmpMNRun::PnlFmncSmpMNRun(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCSMPMNRUN, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncSmpMNRun(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncSmpMNRun::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncSmpMNRun::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncSmpMNRun::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSMPMNRUNDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSMPMNRUNDO) {
...
void PnlFmncSmpMNRun::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncSmpMNRun::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpMNRun::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncSmpMNRun::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncSmpMNRun::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncSmpMNRun::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncSmpMNRun::StgIac::JNUM)) {
...
void PnlFmncSmpMNRun::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQSmpMNRun WHERE ref = " + to_string(refSel), refSel)) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncRun", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngFmncConfirm(false, 0, "");
	else *dpcheng = new DpchEngFmncConfirm(true, jrefNew, "CrdFmncRun");
};

void PnlFmncSmpMNRun::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpMNRun::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSmpMNRun::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncSmpMNRun::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncSmpMNRun::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
