/**
  * \file PnlFmncSepMNSample.cpp
  * job handler for job PnlFmncSepMNSample (implementation)
...
#include "PnlFmncSepMNSample_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncSepMNSample_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncSepMNSample
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncSepMNSample::PnlFmncSepMNSample(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCSEPMNSAMPLE, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncSepMNSample(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncSepMNSample::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncSepMNSample::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncSepMNSample::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncSepMNSample::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSEPMNSAMPLEDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSEPMNSAMPLEDO) {
...
void PnlFmncSepMNSample::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncSepMNSample::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepMNSample::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncSepMNSample::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncSepMNSample::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncSepMNSample::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncSepMNSample::StgIac::JNUM)) {
...
void PnlFmncSepMNSample::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQSepMNSample WHERE ref = " + to_string(refSel), refSel)) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncSmp", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngFmncConfirm(false, 0, "");
	else *dpcheng = new DpchEngFmncConfirm(true, jrefNew, "CrdFmncSmp");
};

void PnlFmncSepMNSample::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepMNSample::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepMNSample::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncSepMNSample::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncSepMNSample::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
