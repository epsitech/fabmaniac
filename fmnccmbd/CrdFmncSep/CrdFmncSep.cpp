/**
  * \file CrdFmncSep.cpp
  * job handler for job CrdFmncSep (implementation)
...
CrdFmncSep::CrdFmncSep(
		...
		) : JobFmnc(xchg, VecFmncVJob::CRDFMNCSEP, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlFmncSepRec(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnlheadbar = new PnlFmncSepHeadbar(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnllist = new PnlFmncSepList(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* CrdFmncSep::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdFmncSep/" + VecFmncVLocale::getSref(ixFmncVLocale);
	continf.MtxCrdSep = StubFmnc::getStubSepStd(dbsfmnc, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFSEP, jref), ixFmncVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdFmncSep::changeRef(
			DbsFmnc* dbsfmnc
	...
	uint ixFmncVAccess;
	xchg->triggerIxRefToIxCall(dbsfmnc, VecFmncVCall::CALLFMNCACCESS, jref, VecFmncVMaintable::TBLFMNCMSTEP, ref, ixFmncVAccess);

	if (ixFmncVAccess != VecFmncVAccess::NONE) {
		xchg->addIxPreset(VecFmncVPreset::PREFMNCIXACC, jref, ixFmncVAccess);

		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsfmnc, VecFmncVCall::CALLFMNCLOG, jref, ixFmncVPreset, xchg->getRefPreset(ixFmncVPreset, jref), "CrdFmncSep", ref);
		xchg->addRefPreset(VecFmncVPreset::PREFMNCREFSEP, jref, ref);

		refresh(dbsfmnc, moditems);
		if (notif) xchg->submitDpch(getNewDpchEng(moditems));

		if (pnllist) pnllist->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFSEP, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFSEP, jrefTrig, notif);
	};
};

void CrdFmncSep::updatePreset(
			DbsFmnc* dbsfmnc
			, const uint ixFmncVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdFmncSep::leaveSgeIdle(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdFmncSep::enterSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
...
void CrdFmncSep::leaveSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeAlrfmncabt --- INSERT
};

uint CrdFmncSep::enterSgeClose(
			DbsFmnc* dbsfmnc
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDCLOSE, jref, VecFmncVCard::CRDFMNCSEP);
	return retval;
};

void CrdFmncSep::leaveSgeClose(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeClose --- INSERT
};

string CrdFmncSep::getSquawk(
			DbsFmnc* dbsfmnc
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdFmncSep::handleRequest(
	...
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSEPDO) {
...
void CrdFmncSep::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void CrdFmncSep::handleDpchAppDoClose(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	muteRefresh = true;
...
void CrdFmncSep::handleDpchAppDoMitAppAbtClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void CrdFmncSep::handleDpchAppFmncAlert(
			DbsFmnc* dbsfmnc
...
bool CrdFmncSep::handleCallFmncDlgClose(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncSep::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncSep::handleCallFmncRefPreSet(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecFmncVPreset::PREFMNCREFSEP) {
