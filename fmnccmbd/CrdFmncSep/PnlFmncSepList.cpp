/**
  * \file PnlFmncSepList.cpp
  * job handler for job PnlFmncSepList (implementation)
...
#include "PnlFmncSepList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncSepList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncSepList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncSepList::PnlFmncSepList(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCSEPLIST, jrefSup, ixFmncVLocale) {
...
// IP constructor.addJob --- END
	...
	QryFmncSepList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecFmncVPreset::PREFMNCREFSEL, jref, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFSEP, jref));

	qry = new QryFmncSepList(xchg, dbsfmnc, jref, ixFmncVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixFmncVExpstate = ((xchg->getRefPreset(VecFmncVPreset::PREFMNCREFSEP, jref)) ? VecFmncVExpstate::MIND : VecFmncVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncSepList::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	...
	if ((ixPre != 0) && (ixPre != VecFmncVPreset::VOID)) {
		...
		if (ixPre == VecFmncVPreset::PREFMNCREFRUN) continf.TxtPre = StubFmnc::getStubRunStd(dbsfmnc, xchg->getRefPreset(ixPre, jref), ixFmncVLocale, Stub::VecVNonetype::FULL);
...
void PnlFmncSepList::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncSepList::regularize(
			DbsFmnc* dbsfmnc
			, const bool notif
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepList::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSEPLISTDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCSEPLISTDO) {
...
void PnlFmncSepList::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncSepList::handleDpchAppDataContiac(
			DbsFmnc* dbsfmnc
			, ContIac* _contiac
			, DpchEngFmnc** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryFmncSepList::VecVOrd::ACT) && (_contiac->numFTos <= QryFmncSepList::VecVOrd::STA)) {
...
void PnlFmncSepList::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepList::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncSepList::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncSepList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncSepList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			refresh(dbsfmnc, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncSepList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsfmnc, VecFmncVCall::CALLFMNCREFPRESET, jref, VecFmncVPreset::PREFMNCREFSEP, refSelNew);
		};
	};
...
void PnlFmncSepList::handleDpchAppDoButMinimizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepList::handleDpchAppDoButRegularizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepList::handleDpchAppDoButNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepList::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepList::handleDpchAppDoButFilterClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncSepList::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncSepList::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncSepList::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
