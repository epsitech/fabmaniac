/**
  * \file QryFmncSepAPar.cpp
  * job handler for job QryFmncSepAPar (implementation)
...
#include "QryFmncSepAPar_blks.cpp"
// IP blksInclude --- END

/******************************************************************************
 class QryFmncSepAPar
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
QryFmncSepAPar::QryFmncSepAPar(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::QRYFMNCSEPAPAR, jrefSup, ixFmncVLocale) {
...
// IP cust --- INSERT
void QryFmncSepAPar::refreshJnum() {
	ubigint preRefSel = xchg->getRefPreset(VecFmncVPreset::PREFMNCREFSEL, jref);

	stgiac.jnum = getJnumByRef(preRefSel);
};

void QryFmncSepAPar::refresh(
			DbsFmnc* dbsfmnc
			, const bool call
		) {
	string sqlstr;
	...
	string preX1 = xchg->getSrefPreset(VecFmncVPreset::PREFMNCSEPAPAR_X1, jref);
	uint preX2 = xchg->getIxPreset(VecFmncVPreset::PREFMNCSEPAPAR_X2, jref);

	xchg->removeClstns(VecFmncVCall::CALLFMNCSTEAPARMOD_STEEQ, jref);

	dbsfmnc->tblfmncqselect->removeRstByJref(jref);
	dbsfmnc->tblfmncqsepapar->removeRstByJref(jref);

	sqlstr = "SELECT COUNT(TblFmncAMStepPar.ref)";
	sqlstr += " FROM TblFmncAMStepPar";
	sqlstr += " WHERE TblFmncAMStepPar.refFmncMStep = " + to_string(preRefSep) + "";
	...
	sqlstr = "INSERT INTO TblFmncQSepAPar(jref, jnum, ref, x1OsrefFmncKSteppar, x2IxVTbl, x2Uref, Val, Comment)";
	...
	sqlstr = "UPDATE TblFmncQSepAPar SET jnum = qref WHERE jref = " + to_string(jref);
	dbsfmnc->executeQuery(sqlstr);
	...
	xchg->addRefClstn(VecFmncVCall::CALLFMNCSTEAPARMOD_STEEQ, jref, Clstn::VecVJobmask::ALL, 0, preRefSep, Clstn::VecVJactype::LOCK);
};

void QryFmncSepAPar::refresh_filtSQL(
			string& sqlstr
			, const string& preX1
			, const uint preX2
			, const bool addwhere
...
void QryFmncSepAPar::refresh_filtSQL_append(
			string& sqlstr
			, bool& first
		) {
	if (first) {
...
void QryFmncSepAPar::fetch(
	...
	dbsfmnc->tblfmncqsepapar->loadRstByJref(jref, false, rst);
	...
	if (stmgr) {
		...
		stmgr->commit();
		stmgr->unlockAccess("QryFmncSepAPar", "fetch");
	};

	refreshJnum();
};

uint QryFmncSepAPar::getJnumByRef(
			const ubigint ref
		) {
	uint retval = 0;
...
ubigint QryFmncSepAPar::getRefByJnum(
			const uint jnum
		) {
	ubigint retval = 0;
...
void QryFmncSepAPar::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
...
bool QryFmncSepAPar::handleRefresh(
			DbsFmnc* dbsfmnc
		) {
	bool retval = false;
	string input;
...
bool QryFmncSepAPar::handleShow(
			DbsFmnc* dbsfmnc
		) {
	bool retval = false;
	FmncQSepAPar* rec = NULL;
...
void QryFmncSepAPar::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool QryFmncSepAPar::handleCallFmncStubChg(
			DbsFmnc* dbsfmnc
...
bool QryFmncSepAPar::handleCallFmncSteAparMod_steEq(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
