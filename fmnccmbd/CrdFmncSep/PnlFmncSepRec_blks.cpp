/**
  * \file PnlFmncSepRec_blks.cpp
  * job handler for job PnlFmncSepRec (implementation of blocks)
...
void PnlFmncSepRec::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncSepRec";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncSepRec";
...
void PnlFmncSepRec::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppFmncSepRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncSepRec";
...
PnlFmncSepRec::StatShr::StatShr(
	...
	mask = {IXFMNCVEXPSTATE, JREFDETAIL, JREFAPAR, JREFREF1NFILE, JREFMNFILE, JREFMNSAMPLE, BUTREGULARIZEACTIVE};
};

void PnlFmncSepRec::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrFmncSepRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncSepRec";
...
set<uint> PnlFmncSepRec::StatShr::diff(
	...
	diffitems = {IXFMNCVEXPSTATE, JREFDETAIL, JREFAPAR, JREFREF1NFILE, JREFMNFILE, JREFMNSAMPLE, BUTREGULARIZEACTIVE};
	for (auto it=commitems.begin();it!=commitems.end();it++) diffitems.erase(*it);
...
void PnlFmncSepRec::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncSepRec";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncSepRec";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Step");
		} else if (ixFmncVLocale == VecFmncVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Schritt");
		} else if (ixFmncVLocale == VecFmncVLocale::FRCH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "\\u00e9tape");
		};
	xmlTextWriterEndElement(wr);
};
...
PnlFmncSepRec::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCSEPRECDO) {
	ixVDo = 0;
};
...
void PnlFmncSepRec::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncSepRecDo");
	else
...
PnlFmncSepRec::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCSEPRECDATA, jref) {
...
void PnlFmncSepRec::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncSepRec::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncSepRecData");
