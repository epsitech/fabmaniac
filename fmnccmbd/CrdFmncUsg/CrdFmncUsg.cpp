/**
  * \file CrdFmncUsg.cpp
  * job handler for job CrdFmncUsg (implementation)
...
CrdFmncUsg::CrdFmncUsg(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
			, const ubigint ref
		) : JobFmnc(xchg, VecFmncVJob::CRDFMNCUSG, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlFmncUsgRec(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnlheadbar = new PnlFmncUsgHeadbar(xchg, dbsfmnc, jref, ixFmncVLocale);
	pnllist = new PnlFmncUsgList(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* CrdFmncUsg::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdFmncUsg/" + VecFmncVLocale::getSref(ixFmncVLocale);
	continf.MtxCrdUsg = StubFmnc::getStubUsgStd(dbsfmnc, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFUSG, jref), ixFmncVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdFmncUsg::changeRef(
			DbsFmnc* dbsfmnc
	...
	uint ixFmncVAccess;
	xchg->triggerIxRefToIxCall(dbsfmnc, VecFmncVCall::CALLFMNCACCESS, jref, VecFmncVMaintable::TBLFMNCMUSERGROUP, ref, ixFmncVAccess);

	if (ixFmncVAccess != VecFmncVAccess::NONE) {
		xchg->addIxPreset(VecFmncVPreset::PREFMNCIXACC, jref, ixFmncVAccess);

		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsfmnc, VecFmncVCall::CALLFMNCLOG, jref, VecFmncVPreset::VOID, 0, "CrdFmncUsg", ref);
		xchg->addRefPreset(VecFmncVPreset::PREFMNCREFUSG, jref, ref);

		refresh(dbsfmnc, moditems);
		if (notif) xchg->submitDpch(getNewDpchEng(moditems));

		if (pnllist) pnllist->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFUSG, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbsfmnc, VecFmncVPreset::PREFMNCREFUSG, jrefTrig, notif);
	};
};

void CrdFmncUsg::updatePreset(
			DbsFmnc* dbsfmnc
...
void CrdFmncUsg::leaveSgeIdle(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdFmncUsg::enterSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
...
void CrdFmncUsg::leaveSgeAlrfmncabt(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeAlrfmncabt --- INSERT
};

uint CrdFmncUsg::enterSgeClose(
			DbsFmnc* dbsfmnc
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDCLOSE, jref, VecFmncVCard::CRDFMNCUSG);
	return retval;
};

void CrdFmncUsg::leaveSgeClose(
			DbsFmnc* dbsfmnc
		) {
// IP leaveSgeClose --- INSERT
};

string CrdFmncUsg::getSquawk(
			DbsFmnc* dbsfmnc
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdFmncUsg::handleRequest(
	...
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCUSGDO) {
...
void CrdFmncUsg::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void CrdFmncUsg::handleDpchAppDoClose(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	muteRefresh = true;
...
void CrdFmncUsg::handleDpchAppDoMitAppAbtClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void CrdFmncUsg::handleDpchAppFmncAlert(
			DbsFmnc* dbsfmnc
...
bool CrdFmncUsg::handleCallFmncDlgClose(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncUsg::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdFmncUsg::handleCallFmncRefPreSet(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecFmncVPreset::PREFMNCREFUSG) {
