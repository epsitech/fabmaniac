/**
  * \file PnlFmncPrsMNOrg_blks.cpp
  * job handler for job PnlFmncPrsMNOrg (implementation of blocks)
...
void PnlFmncPrsMNOrg::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncPrsMNOrg";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncPrsMNOrg";
...
void PnlFmncPrsMNOrg::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncPrsMNOrg";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncPrsMNOrg";
...
void PnlFmncPrsMNOrg::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncPrsMNOrg";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncPrsMNOrg";
...
void PnlFmncPrsMNOrg::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncPrsMNOrg";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncPrsMNOrg";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Organizations");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Organization");
			writeStringAttr(wr, itemtag, "sref", "TcoSta", "X1Startd");
			writeStringAttr(wr, itemtag, "sref", "TcoSto", "X1Stopd");
			writeStringAttr(wr, itemtag, "sref", "TcoFct", "Function");
		} else if (ixFmncVLocale == VecFmncVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Organisationen");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Organisation");
			writeStringAttr(wr, itemtag, "sref", "TcoSta", "X1Startd");
			writeStringAttr(wr, itemtag, "sref", "TcoSto", "X1Stopd");
			writeStringAttr(wr, itemtag, "sref", "TcoFct", "Funktion");
		} else if (ixFmncVLocale == VecFmncVLocale::FRCH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Organismes");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Organisme");
			writeStringAttr(wr, itemtag, "sref", "TcoSta", "X1Startd");
			writeStringAttr(wr, itemtag, "sref", "TcoSto", "X1Stopd");
			writeStringAttr(wr, itemtag, "sref", "TcoFct", "Fonction");
		};
...
PnlFmncPrsMNOrg::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCPRSMNORGDATA) {
};
...
void PnlFmncPrsMNOrg::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncPrsMNOrgData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncPrsMNOrg::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCPRSMNORGDO) {
	ixVDo = 0;
};
...
void PnlFmncPrsMNOrg::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncPrsMNOrgDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncPrsMNOrg::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryFmncPrsMNOrg::StatShr* statshrqry
			, QryFmncPrsMNOrg::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCPRSMNORGDATA, jref) {
...
void PnlFmncPrsMNOrg::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncPrsMNOrg::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncPrsMNOrgData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncPrsMNOrg::StatApp::writeXML(wr);
