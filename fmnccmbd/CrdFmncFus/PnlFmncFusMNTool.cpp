/**
  * \file PnlFmncFusMNTool.cpp
  * job handler for job PnlFmncFusMNTool (implementation)
...
#include "PnlFmncFusMNTool_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncFusMNTool_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncFusMNTool
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncFusMNTool::PnlFmncFusMNTool(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCFUSMNTOOL, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncFusMNTool(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncFusMNTool::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncFusMNTool::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncFusMNTool::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFUSMNTOOLDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFUSMNTOOLDO) {
...
void PnlFmncFusMNTool::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncFusMNTool::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFusMNTool::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncFusMNTool::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncFusMNTool::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncFusMNTool::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncFusMNTool::StgIac::JNUM)) {
...
void PnlFmncFusMNTool::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQFusMNTool WHERE ref = " + to_string(refSel), refSel)) {
		if (ixPre == VecFmncVPreset::PREFMNCREFFAB) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, ixPre, xchg->getRefPreset(ixPre, jref), "CrdFmncTol", refSel, jrefNew);
		else xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncTol", refSel, jrefNew);
	};
...
void PnlFmncFusMNTool::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFusMNTool::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFusMNTool::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncFusMNTool::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncFusMNTool::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
