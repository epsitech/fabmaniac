/**
  * \file PnlFmncFusMNFabproject_blks.cpp
  * job handler for job PnlFmncFusMNFabproject (implementation of blocks)
...
void PnlFmncFusMNFabproject::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncFusMNFabproject";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncFusMNFabproject";
...
void PnlFmncFusMNFabproject::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncFusMNFabproject";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncFusMNFabproject";
...
void PnlFmncFusMNFabproject::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncFusMNFabproject";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncFusMNFabproject";
...
void PnlFmncFusMNFabproject::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncFusMNFabproject";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncFusMNFabproject";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Fab projects");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Fab project");
		} else if (ixFmncVLocale == VecFmncVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Reinraumprojekte");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Reinraumprojekt");
		} else if (ixFmncVLocale == VecFmncVLocale::FRCH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Projets salle blanche");
			writeStringAttr(wr, itemtag, "sref", "TcoMref", "Projet salle blanche");
		};
...
PnlFmncFusMNFabproject::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFUSMNFABPROJECTDATA) {
};
...
void PnlFmncFusMNFabproject::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFusMNFabprojectData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFusMNFabproject::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFUSMNFABPROJECTDO) {
	ixVDo = 0;
};
...
void PnlFmncFusMNFabproject::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFusMNFabprojectDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFusMNFabproject::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCFUSMNFABPROJECTDATA, jref) {
...
void PnlFmncFusMNFabproject::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncFusMNFabproject::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncFusMNFabprojectData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncFusMNFabproject::StatApp::writeXML(wr);
