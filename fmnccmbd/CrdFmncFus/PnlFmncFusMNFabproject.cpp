/**
  ...
  * job handler for job PnlFmncFusMNFabproject (implementation)
...
#include "PnlFmncFusMNFabproject_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncFusMNFabproject_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncFusMNFabproject
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncFusMNFabproject::PnlFmncFusMNFabproject(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCFUSMNFABPROJECT, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncFusMNFabproject(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncFusMNFabproject::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncFusMNFabproject::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncFusMNFabproject::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncFusMNFabproject::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFUSMNFABPROJECTDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFUSMNFABPROJECTDO) {
...
void PnlFmncFusMNFabproject::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncFusMNFabproject::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFusMNFabproject::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncFusMNFabproject::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncFusMNFabproject::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncFusMNFabproject::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncFusMNFabproject::StgIac::JNUM)) {
...
void PnlFmncFusMNFabproject::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQFusMNFabproject WHERE ref = " + to_string(refSel), refSel)) {
		if (ixPre == VecFmncVPreset::PREFMNCREFFAB) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, ixPre, xchg->getRefPreset(ixPre, jref), "CrdFmncFpj", refSel, jrefNew);
		else xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncFpj", refSel, jrefNew);
	};
...
void PnlFmncFusMNFabproject::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFusMNFabproject::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFusMNFabproject::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncFusMNFabproject::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncFusMNFabproject::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
