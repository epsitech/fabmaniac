/**
  * \file PnlFmncTolRlv1NStep.cpp
  * job handler for job PnlFmncTolRlv1NStep (implementation)
...
#include "PnlFmncTolRlv1NStep_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncTolRlv1NStep_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncTolRlv1NStep
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncTolRlv1NStep::PnlFmncTolRlv1NStep(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCTOLRLV1NSTEP, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncTolRlv1NStep(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncTolRlv1NStep::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncTolRlv1NStep::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncTolRlv1NStep::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncTolRlv1NStep::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCTOLRLV1NSTEPDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCTOLRLV1NSTEPDO) {
...
void PnlFmncTolRlv1NStep::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncTolRlv1NStep::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncTolRlv1NStep::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncTolRlv1NStep::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncTolRlv1NStep::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncTolRlv1NStep::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncTolRlv1NStep::StgIac::JNUM)) {
...
void PnlFmncTolRlv1NStep::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncTolRlv1NStep::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncTolRlv1NStep::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncTolRlv1NStep::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
