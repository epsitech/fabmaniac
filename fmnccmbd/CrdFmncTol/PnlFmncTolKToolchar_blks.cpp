/**
  * \file PnlFmncTolKToolchar_blks.cpp
  * job handler for job PnlFmncTolKToolchar (implementation of blocks)
...
void PnlFmncTolKToolchar::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncTolKToolchar";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncTolKToolchar";
...
void PnlFmncTolKToolchar::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncTolKToolchar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncTolKToolchar";
...
void PnlFmncTolKToolchar::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncTolKToolchar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncTolKToolchar";
...
void PnlFmncTolKToolchar::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncTolKToolchar";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncTolKToolchar";
...
PnlFmncTolKToolchar::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTOLKTOOLCHARDATA) {
};
...
void PnlFmncTolKToolchar::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTolKToolcharData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTolKToolchar::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTOLKTOOLCHARDO) {
	ixVDo = 0;
};
...
void PnlFmncTolKToolchar::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTolKToolcharDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTolKToolchar::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryFmncTolKToolchar::StatShr* statshrqry
			, QryFmncTolKToolchar::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCTOLKTOOLCHARDATA, jref) {
...
void PnlFmncTolKToolchar::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncTolKToolchar::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncTolKToolcharData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncTolKToolchar::StatApp::writeXML(wr);
