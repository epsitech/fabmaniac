/**
  * \file PnlFmncTolAChar_blks.cpp
  * job handler for job PnlFmncTolAChar (implementation of blocks)
...
void PnlFmncTolAChar::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncTolAChar";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncTolAChar";
...
void PnlFmncTolAChar::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncTolAChar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncTolAChar";
...
void PnlFmncTolAChar::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncTolAChar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncTolAChar";
...
void PnlFmncTolAChar::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncTolAChar";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncTolAChar";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Characteristics");
			writeStringAttr(wr, itemtag, "sref", "TcoTch", "Characteristic");
			writeStringAttr(wr, itemtag, "sref", "TcoVal", "Value");
			writeStringAttr(wr, itemtag, "sref", "TcoCmt", "Comment");
		} else if (ixFmncVLocale == VecFmncVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Eigenschaften");
			writeStringAttr(wr, itemtag, "sref", "TcoTch", "Eigenschaft");
			writeStringAttr(wr, itemtag, "sref", "TcoVal", "Wert");
			writeStringAttr(wr, itemtag, "sref", "TcoCmt", "Notiz");
		} else if (ixFmncVLocale == VecFmncVLocale::FRCH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Propri\\u00e9t\\u00e9s");
			writeStringAttr(wr, itemtag, "sref", "TcoTch", "Caract\\u00e9ristique");
...
PnlFmncTolAChar::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTOLACHARDATA) {
};
...
void PnlFmncTolAChar::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTolACharData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTolAChar::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTOLACHARDO) {
	ixVDo = 0;
};
...
void PnlFmncTolAChar::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTolACharDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTolAChar::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCTOLACHARDATA, jref) {
...
void PnlFmncTolAChar::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncTolAChar::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncTolACharData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncTolAChar::StatApp::writeXML(wr);
