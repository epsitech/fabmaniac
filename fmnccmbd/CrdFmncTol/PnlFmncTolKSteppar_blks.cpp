/**
  * \file PnlFmncTolKSteppar_blks.cpp
  * job handler for job PnlFmncTolKSteppar (implementation of blocks)
...
void PnlFmncTolKSteppar::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncTolKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncTolKSteppar";
...
void PnlFmncTolKSteppar::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixFmncVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppFmncTolKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncTolKSteppar";
...
void PnlFmncTolKSteppar::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncTolKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncTolKSteppar";
...
void PnlFmncTolKSteppar::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncTolKSteppar";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncTolKSteppar";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Param\\u00e8tre");
...
PnlFmncTolKSteppar::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTOLKSTEPPARDATA) {
};
...
void PnlFmncTolKSteppar::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTolKStepparData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTolKSteppar::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTOLKSTEPPARDO) {
	ixVDo = 0;
};
...
void PnlFmncTolKSteppar::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncTolKStepparDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncTolKSteppar::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryFmncTolKSteppar::StatShr* statshrqry
			, QryFmncTolKSteppar::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCTOLKSTEPPARDATA, jref) {
...
void PnlFmncTolKSteppar::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncTolKSteppar::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncTolKStepparData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncTolKSteppar::StatApp::writeXML(wr);
