/**
  * \file PnlFmncPcoList_blks.cpp
  * job handler for job PnlFmncPcoList (implementation of blocks)
...
void PnlFmncPcoList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncPcoList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncPcoList";
...
void PnlFmncPcoList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncPcoList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncPcoList";
...
void PnlFmncPcoList::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncPcoList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncPcoList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoPco", "Client organization");
			writeStringAttr(wr, itemtag, "sref", "TcoSpl", "Supplier");
			writeStringAttr(wr, itemtag, "sref", "TcoFil", "Purchase order");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoPco", "Kundenorganisation");
			writeStringAttr(wr, itemtag, "sref", "TcoSpl", "Lieferant");
			writeStringAttr(wr, itemtag, "sref", "TcoFil", "Bestellung");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoDat", "Date");
			writeStringAttr(wr, itemtag, "sref", "TcoPco", "Organisme client");
			writeStringAttr(wr, itemtag, "sref", "TcoSpl", "Fournisseur");
			writeStringAttr(wr, itemtag, "sref", "TcoFil", "Commande");
...
PnlFmncPcoList::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCPCOLISTDATA) {
};
...
void PnlFmncPcoList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncPcoListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncPcoList::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCPCOLISTDO) {
	ixVDo = 0;
};
...
void PnlFmncPcoList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncPcoListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncPcoList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCPCOLISTDATA, jref) {
...
void PnlFmncPcoList::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncPcoList::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncPcoListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncPcoList::StatApp::writeXML(wr);
