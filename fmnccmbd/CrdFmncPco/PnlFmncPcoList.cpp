/**
  ...
  * job handler for job PnlFmncPcoList (implementation)
...
#include "PnlFmncPcoList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncPcoList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncPcoList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncPcoList::PnlFmncPcoList(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCPCOLIST, jrefSup, ixFmncVLocale) {
...
// IP constructor.addJob --- END
	...
	QryFmncPcoList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecFmncVPreset::PREFMNCREFSEL, jref, xchg->getRefPreset(VecFmncVPreset::PREFMNCREFPCO, jref));
	xchg->addIxPreset(VecFmncVPreset::PREFMNCIXORD, jref, QryFmncPcoList::VecVOrd::SRF);

	qry = new QryFmncPcoList(xchg, dbsfmnc, jref, ixFmncVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixFmncVExpstate = ((xchg->getRefPreset(VecFmncVPreset::PREFMNCREFPCO, jref)) ? VecFmncVExpstate::MIND : VecFmncVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncPcoList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncPcoList::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncPcoList::regularize(
			DbsFmnc* dbsfmnc
			, const bool notif
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncPcoList::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCPCOLISTDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCPCOLISTDO) {
...
void PnlFmncPcoList::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncPcoList::handleDpchAppDataContiac(
			DbsFmnc* dbsfmnc
			, ContIac* _contiac
			, DpchEngFmnc** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryFmncPcoList::VecVOrd::SRF) && (_contiac->numFTos <= QryFmncPcoList::VecVOrd::STE)) {
...
void PnlFmncPcoList::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncPcoList::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncPcoList::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncPcoList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncPcoList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			refresh(dbsfmnc, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncPcoList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsfmnc, VecFmncVCall::CALLFMNCREFPRESET, jref, VecFmncVPreset::PREFMNCREFPCO, refSelNew);
		};
	};
...
void PnlFmncPcoList::handleDpchAppDoButMinimizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncPcoList::handleDpchAppDoButRegularizeClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncPcoList::handleDpchAppDoButNewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncPcoList::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncPcoList::handleDpchAppDoButFilterClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncPcoList::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncPcoList::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncPcoList::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
