/**
  * \file PnlFmncFpjMNFabuser.cpp
  * job handler for job PnlFmncFpjMNFabuser (implementation)
...
#include "PnlFmncFpjMNFabuser_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncFpjMNFabuser_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncFpjMNFabuser
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncFpjMNFabuser::PnlFmncFpjMNFabuser(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCFPJMNFABUSER, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncFpjMNFabuser(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncFpjMNFabuser::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncFpjMNFabuser::refresh(
			DbsFmnc* dbsfmnc
			, set<uint>& moditems
		) {
...
void PnlFmncFpjMNFabuser::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncFpjMNFabuser::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFPJMNFABUSERDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCFPJMNFABUSERDO) {
...
void PnlFmncFpjMNFabuser::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncFpjMNFabuser::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFpjMNFabuser::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncFpjMNFabuser::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncFpjMNFabuser::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncFpjMNFabuser::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncFpjMNFabuser::StgIac::JNUM)) {
...
void PnlFmncFpjMNFabuser::handleDpchAppDoButViewClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsfmnc->loadUbigintBySQL("SELECT mref FROM TblFmncQFpjMNFabuser WHERE ref = " + to_string(refSel), refSel)) {
		if (ixPre == VecFmncVPreset::PREFMNCREFFAB) xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, ixPre, xchg->getRefPreset(ixPre, jref), "CrdFmncFus", refSel, jrefNew);
		else xchg->triggerIxRefSrefIntvalToRefCall(dbsfmnc, VecFmncVCall::CALLFMNCCRDOPEN, jref, 0, 0, "CrdFmncFus", refSel, jrefNew);
	};
...
void PnlFmncFpjMNFabuser::handleDpchAppDoButAddClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFpjMNFabuser::handleDpchAppDoButSubClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncFpjMNFabuser::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncFpjMNFabuser::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncFpjMNFabuser::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
