/**
  * \file PnlFmncFpjList_blks.cpp
  * job handler for job PnlFmncFpjList (implementation of blocks)
...
void PnlFmncFpjList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncFpjList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncFpjList";
...
void PnlFmncFpjList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrFmncFpjList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncFpjList";
...
void PnlFmncFpjList::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncFpjList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncFpjList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixFmncVLocale == VecFmncVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoBio", "Organization for billing");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoBio", "Organisation zur Rechnungsstellung");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoBio", "Organisme pour facturisation");
...
PnlFmncFpjList::DpchAppData::DpchAppData() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFPJLISTDATA) {
};
...
void PnlFmncFpjList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFpjListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFpjList::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFPJLISTDO) {
	ixVDo = 0;
};
...
void PnlFmncFpjList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncFpjListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlFmncFpjList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCFPJLISTDATA, jref) {
...
void PnlFmncFpjList::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void PnlFmncFpjList::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncFpjListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		...
		if (has(STATAPPQRY)) QryFmncFpjList::StatApp::writeXML(wr);
