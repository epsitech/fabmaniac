/**
  * \file CrdFmncAct_blks.cpp
  * job handler for job CrdFmncAct (implementation of blocks)
...
void CrdFmncAct::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdFmncAct::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfFmncAct";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfFmncAct";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdAct", MtxCrdAct);
...
set<uint> CrdFmncAct::ContInf::comm(
	...
	if (MtxCrdAct == comp->MtxCrdAct) insert(items, MTXCRDACT);
...
void CrdFmncAct::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppFmncAct";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppFmncAct";
...
void CrdFmncAct::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrFmncAct";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrFmncAct";
...
void CrdFmncAct::Tag::writeXML(
			const uint ixFmncVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagFmncAct";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemFmncAct";
...
CrdFmncAct::DpchAppDo::DpchAppDo() : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCACTDO) {
	ixVDo = 0;
};
...
void CrdFmncAct::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppFmncActDo");
	else
...
CrdFmncAct::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFSge
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCACTDATA, jref) {
...
void CrdFmncAct::DpchEngData::merge(
			DpchEngFmnc* dpcheng
		) {
...
void CrdFmncAct::DpchEngData::writeXML(
			const uint ixFmncVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngFmncActData");
