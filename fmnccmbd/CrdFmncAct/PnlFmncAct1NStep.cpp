/**
  * \file PnlFmncAct1NStep.cpp
  * job handler for job PnlFmncAct1NStep (implementation)
...
#include "PnlFmncAct1NStep_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlFmncAct1NStep_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlFmncAct1NStep
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlFmncAct1NStep::PnlFmncAct1NStep(
			XchgFmnc* xchg
			, DbsFmnc* dbsfmnc
			, const ubigint jrefSup
			, const uint ixFmncVLocale
		) : JobFmnc(xchg, VecFmncVJob::PNLFMNCACT1NSTEP, jrefSup, ixFmncVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryFmncAct1NStep(xchg, dbsfmnc, jref, ixFmncVLocale);
...
// IP cust --- INSERT
DpchEngFmnc* PnlFmncAct1NStep::getNewDpchEng(
			set<uint> items
		) {
...
void PnlFmncAct1NStep::updatePreset(
			DbsFmnc* dbsfmnc
...
void PnlFmncAct1NStep::handleRequest(
			DbsFmnc* dbsfmnc
			, ReqFmnc* req
		) {
	if (req->ixVBasetype == ReqFmnc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCACT1NSTEPDATA) {
		...
		} else if (req->dpchapp->ixFmncVDpch == VecFmncVDpch::DPCHAPPFMNCACT1NSTEPDO) {
...
void PnlFmncAct1NStep::handleDpchAppFmncInit(
			DbsFmnc* dbsfmnc
...
void PnlFmncAct1NStep::handleDpchAppDataStgiac(
			DbsFmnc* dbsfmnc
			, StgIac* _stgiac
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncAct1NStep::handleDpchAppDataStgiacqry(
			DbsFmnc* dbsfmnc
			, QryFmncAct1NStep::StgIac* _stgiacqry
			, DpchEngFmnc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryFmncAct1NStep::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryFmncAct1NStep::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsfmnc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryFmncAct1NStep::StgIac::JNUM)) {
...
void PnlFmncAct1NStep::handleDpchAppDoButDeleteClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
...
void PnlFmncAct1NStep::handleDpchAppDoButRefreshClick(
			DbsFmnc* dbsfmnc
			, DpchEngFmnc** dpcheng
		) {
	set<uint> moditems;
...
void PnlFmncAct1NStep::handleCall(
			DbsFmnc* dbsfmnc
			, Call* call
		) {
...
bool PnlFmncAct1NStep::handleCallFmncStatChg(
			DbsFmnc* dbsfmnc
			, const ubigint jrefTrig
		) {
	bool retval = false;
