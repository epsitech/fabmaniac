/**
  ...
  * API code for job PnlFmncFusList (implementation)
...
bool PnlFmncFusList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncFusList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncFusList";
...
bool PnlFmncFusList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncFusList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncFusList";
...
PnlFmncFusList::Tag::Tag(
	...
	mask = {CPT, TXTFOR, TRS, TXTSHOWING1, TXTSHOWING2, TCOPRS, TCOSRF, TCOFAB, TCOFPJ, TCOSTE};
};

bool PnlFmncFusList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncFusList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncFusList";
...
PnlFmncFusList::DpchAppData::DpchAppData(
			const string& scrJref
			, ContIac* contiac
			, StgIac* stgiac
			, QryFmncFusList::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFUSLISTDATA, scrJref) {
...
void PnlFmncFusList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncFusListData");
...
PnlFmncFusList::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFUSLISTDO, scrJref) {
...
void PnlFmncFusList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncFusListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlFmncFusList::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCFUSLISTDATA) {
...
void PnlFmncFusList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncFusListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryFmncFusList::StatApp();
		statshrqry = QryFmncFusList::StatShr();
