/**
  ...
  * API code for job QryFmncBilList (implementation)
...
/******************************************************************************
 class QryFmncBilList::VecVOrd
 ******************************************************************************/

uint QryFmncBilList::VecVOrd::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);

	if (s.compare("srf") == 0) return SRF;
	else if (s.compare("dat") == 0) return DAT;
	else if (s.compare("prj") == 0) return PRJ;
	else if (s.compare("fil") == 0) return FIL;
	else if (s.compare("ste") == 0) return STE;

	return(0);
};

string QryFmncBilList::VecVOrd::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class QryFmncBilList::StatApp
 ******************************************************************************/

QryFmncBilList::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncBilList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncBilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncBilList";
...
set<uint> QryFmncBilList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncBilList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncBilList::StatShr
 ******************************************************************************/

QryFmncBilList::StatShr::StatShr(
			const uint ntot
...
bool QryFmncBilList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncBilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncBilList";
...
set<uint> QryFmncBilList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryFmncBilList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncBilList::StgIac
 ******************************************************************************/

QryFmncBilList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryFmncBilList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryFmncBilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryFmncBilList";
...
void QryFmncBilList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryFmncBilList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryFmncBilList";
...
set<uint> QryFmncBilList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryFmncBilList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
