/**
  ...
  * API code for job QryFmncOrgMNArticle (implementation)
...
QryFmncOrgMNArticle::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncOrgMNArticle::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncOrgMNArticle");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncOrgMNArticle";
...
set<uint> QryFmncOrgMNArticle::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncOrgMNArticle::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryFmncOrgMNArticle::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncOrgMNArticle");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncOrgMNArticle";
