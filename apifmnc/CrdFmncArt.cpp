/**
  * \file CrdFmncArt.cpp
  * API code for job CrdFmncArt (implementation)
...
bool CrdFmncArt::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncArt");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncArt";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdArt", MtxCrdArt)) add(MTXCRDART);
	};
...
set<uint> CrdFmncArt::ContInf::comm(
	...
	if (MtxCrdArt.compare(comp->MtxCrdArt) == 0) insert(items, MTXCRDART);
...
bool CrdFmncArt::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppFmncArt");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppFmncArt";
...
set<uint> CrdFmncArt::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdFmncArt::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdFmncArt::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncArt");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncArt";
...
bool CrdFmncArt::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncArt");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncArt";
...
CrdFmncArt::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCARTDO, scrJref) {
...
void CrdFmncArt::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncArtDo");
...
CrdFmncArt::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCARTDATA) {
...
void CrdFmncArt::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncArtData");
	else
		basefound = checkXPath(docctx, basexpath);
