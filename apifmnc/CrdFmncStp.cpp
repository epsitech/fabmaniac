/**
  * \file CrdFmncStp.cpp
  * API code for job CrdFmncStp (implementation)
...
bool CrdFmncStp::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncStp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncStp";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdStp", MtxCrdStp)) add(MTXCRDSTP);
	};
...
set<uint> CrdFmncStp::ContInf::comm(
	...
	if (MtxCrdStp.compare(comp->MtxCrdStp) == 0) insert(items, MTXCRDSTP);
...
bool CrdFmncStp::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppFmncStp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppFmncStp";
...
set<uint> CrdFmncStp::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdFmncStp::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdFmncStp::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncStp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncStp";
...
bool CrdFmncStp::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncStp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncStp";
...
CrdFmncStp::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCSTPDO, scrJref) {
...
void CrdFmncStp::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncStpDo");
...
CrdFmncStp::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCSTPDATA) {
...
void CrdFmncStp::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncStpData");
	else
		basefound = checkXPath(docctx, basexpath);
