/**
  * \file PnlFmncAdrHeadbar.cpp
  * API code for job PnlFmncAdrHeadbar (implementation)
...
PnlFmncAdrHeadbar::StgInf::StgInf(
...
bool PnlFmncAdrHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfFmncAdrHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfFmncAdrHeadbar";
...
set<uint> PnlFmncAdrHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlFmncAdrHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlFmncAdrHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncAdrHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncAdrHeadbar";
...
PnlFmncAdrHeadbar::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCADRHEADBARDATA) {
};
...
void PnlFmncAdrHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncAdrHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
