/**
  ...
  * API code for job QryFmncSmpMNRun (implementation)
...
QryFmncSmpMNRun::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncSmpMNRun::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncSmpMNRun");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncSmpMNRun";
...
set<uint> QryFmncSmpMNRun::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncSmpMNRun::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryFmncSmpMNRun::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncSmpMNRun");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncSmpMNRun";
