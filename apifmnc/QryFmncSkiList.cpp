/**
  ...
  * API code for job QryFmncSkiList (implementation)
...
/******************************************************************************
 class QryFmncSkiList::VecVOrd
 ******************************************************************************/

uint QryFmncSkiList::VecVOrd::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);

	if (s.compare("org") == 0) return ORG;
	else if (s.compare("art") == 0) return ART;

	return(0);
};

string QryFmncSkiList::VecVOrd::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class QryFmncSkiList::StatApp
 ******************************************************************************/

QryFmncSkiList::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncSkiList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncSkiList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncSkiList";
...
set<uint> QryFmncSkiList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncSkiList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncSkiList::StatShr
 ******************************************************************************/

QryFmncSkiList::StatShr::StatShr(
			const uint ntot
...
bool QryFmncSkiList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncSkiList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncSkiList";
...
set<uint> QryFmncSkiList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryFmncSkiList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncSkiList::StgIac
 ******************************************************************************/

QryFmncSkiList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryFmncSkiList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryFmncSkiList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryFmncSkiList";
...
void QryFmncSkiList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryFmncSkiList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryFmncSkiList";
...
set<uint> QryFmncSkiList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryFmncSkiList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
