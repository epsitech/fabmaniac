/**
  ...
  * API code for job PnlFmncUsgAAccess (implementation)
...
bool PnlFmncUsgAAccess::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncUsgAAccess";
...
PnlFmncUsgAAccess::StatApp::StatApp(
...
bool PnlFmncUsgAAccess::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppFmncUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppFmncUsgAAccess";
...
set<uint> PnlFmncUsgAAccess::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlFmncUsgAAccess::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlFmncUsgAAccess::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncUsgAAccess";
...
bool PnlFmncUsgAAccess::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncUsgAAccess";
...
PnlFmncUsgAAccess::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryFmncUsgAAccess::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCUSGAACCESSDATA, scrJref) {
...
void PnlFmncUsgAAccess::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncUsgAAccessData");
...
PnlFmncUsgAAccess::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCUSGAACCESSDO, scrJref) {
...
void PnlFmncUsgAAccess::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncUsgAAccessDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlFmncUsgAAccess::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCUSGAACCESSDATA) {
...
void PnlFmncUsgAAccess::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncUsgAAccessData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryFmncUsgAAccess::StatApp();
		statshrqry = QryFmncUsgAAccess::StatShr();
