/**
  ...
  * API code for job QryFmncArtMNOrg (implementation)
...
/******************************************************************************
 class QryFmncArtMNOrg::StatApp
 ******************************************************************************/

QryFmncArtMNOrg::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncArtMNOrg::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncArtMNOrg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncArtMNOrg";
...
set<uint> QryFmncArtMNOrg::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncArtMNOrg::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncArtMNOrg::StatShr
 ******************************************************************************/

QryFmncArtMNOrg::StatShr::StatShr(
			const uint ntot
...
bool QryFmncArtMNOrg::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncArtMNOrg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncArtMNOrg";
...
set<uint> QryFmncArtMNOrg::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryFmncArtMNOrg::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncArtMNOrg::StgIac
 ******************************************************************************/

QryFmncArtMNOrg::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryFmncArtMNOrg::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryFmncArtMNOrg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryFmncArtMNOrg";
...
void QryFmncArtMNOrg::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryFmncArtMNOrg";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryFmncArtMNOrg";
...
set<uint> QryFmncArtMNOrg::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryFmncArtMNOrg::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
