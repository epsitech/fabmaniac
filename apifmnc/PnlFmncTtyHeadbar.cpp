/**
  * \file PnlFmncTtyHeadbar.cpp
  * API code for job PnlFmncTtyHeadbar (implementation)
...
PnlFmncTtyHeadbar::StgInf::StgInf(
...
bool PnlFmncTtyHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfFmncTtyHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfFmncTtyHeadbar";
...
set<uint> PnlFmncTtyHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlFmncTtyHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlFmncTtyHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncTtyHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncTtyHeadbar";
...
PnlFmncTtyHeadbar::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCTTYHEADBARDATA) {
};
...
void PnlFmncTtyHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncTtyHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
