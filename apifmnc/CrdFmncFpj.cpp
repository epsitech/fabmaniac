/**
  * \file CrdFmncFpj.cpp
  * API code for job CrdFmncFpj (implementation)
...
bool CrdFmncFpj::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncFpj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncFpj";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdFpj", MtxCrdFpj)) add(MTXCRDFPJ);
	};
...
set<uint> CrdFmncFpj::ContInf::comm(
	...
	if (MtxCrdFpj.compare(comp->MtxCrdFpj) == 0) insert(items, MTXCRDFPJ);
...
bool CrdFmncFpj::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppFmncFpj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppFmncFpj";
...
set<uint> CrdFmncFpj::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdFmncFpj::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdFmncFpj::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncFpj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncFpj";
...
bool CrdFmncFpj::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncFpj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncFpj";
...
CrdFmncFpj::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCFPJDO, scrJref) {
...
void CrdFmncFpj::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncFpjDo");
...
CrdFmncFpj::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCFPJDATA) {
...
void CrdFmncFpj::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncFpjData");
	else
		basefound = checkXPath(docctx, basexpath);
