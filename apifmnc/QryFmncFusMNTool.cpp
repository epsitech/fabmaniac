/**
  ...
  * API code for job QryFmncFusMNTool (implementation)
...
/******************************************************************************
 class QryFmncFusMNTool::StatApp
 ******************************************************************************/

QryFmncFusMNTool::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncFusMNTool::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncFusMNTool");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncFusMNTool";
...
set<uint> QryFmncFusMNTool::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncFusMNTool::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncFusMNTool::StatShr
 ******************************************************************************/

QryFmncFusMNTool::StatShr::StatShr(
			const uint ntot
...
bool QryFmncFusMNTool::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncFusMNTool");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncFusMNTool";
...
set<uint> QryFmncFusMNTool::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryFmncFusMNTool::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncFusMNTool::StgIac
 ******************************************************************************/

QryFmncFusMNTool::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryFmncFusMNTool::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryFmncFusMNTool");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryFmncFusMNTool";
...
void QryFmncFusMNTool::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryFmncFusMNTool";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryFmncFusMNTool";
...
set<uint> QryFmncFusMNTool::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryFmncFusMNTool::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
