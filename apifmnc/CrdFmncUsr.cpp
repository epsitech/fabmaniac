/**
  * \file CrdFmncUsr.cpp
  * API code for job CrdFmncUsr (implementation)
...
bool CrdFmncUsr::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncUsr";
...
bool CrdFmncUsr::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppFmncUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppFmncUsr";
...
set<uint> CrdFmncUsr::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdFmncUsr::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdFmncUsr::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncUsr";
...
bool CrdFmncUsr::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncUsr";
...
CrdFmncUsr::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCUSRDO, scrJref) {
...
void CrdFmncUsr::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncUsrDo");
...
CrdFmncUsr::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCUSRDATA) {
...
void CrdFmncUsr::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncUsrData");
	else
		basefound = checkXPath(docctx, basexpath);
