/**
  ...
  * API code for job QryFmncFab1NFabproject (implementation)
...
QryFmncFab1NFabproject::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncFab1NFabproject::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncFab1NFabproject");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncFab1NFabproject";
...
set<uint> QryFmncFab1NFabproject::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncFab1NFabproject::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryFmncFab1NFabproject::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncFab1NFabproject");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncFab1NFabproject";
