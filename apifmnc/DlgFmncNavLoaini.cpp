/**
  ...
  * API code for job DlgFmncNavLoaini (implementation)
...
bool DlgFmncNavLoaini::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfDlgFmncNavLoaini");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfDlgFmncNavLoaini";
...
bool DlgFmncNavLoaini::ContInfImp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfDlgFmncNavLoainiImp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfDlgFmncNavLoainiImp";
...
DlgFmncNavLoaini::StatApp::StatApp(
			const bool initdone
...
bool DlgFmncNavLoaini::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppDlgFmncNavLoaini");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppDlgFmncNavLoaini";
...
set<uint> DlgFmncNavLoaini::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> DlgFmncNavLoaini::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool DlgFmncNavLoaini::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrDlgFmncNavLoaini");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrDlgFmncNavLoaini";
...
bool DlgFmncNavLoaini::StatShrIfi::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrDlgFmncNavLoainiIfi");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrDlgFmncNavLoainiIfi";
...
bool DlgFmncNavLoaini::StatShrImp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrDlgFmncNavLoainiImp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrDlgFmncNavLoainiImp";
...
bool DlgFmncNavLoaini::StatShrLfi::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrDlgFmncNavLoainiLfi");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrDlgFmncNavLoainiLfi";
...
bool DlgFmncNavLoaini::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagDlgFmncNavLoaini");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemDlgFmncNavLoaini";
...
DlgFmncNavLoaini::TagIfi::TagIfi(
			const string& Uld
			, const string& Cpt
		) : Block() {
	this->Uld = Uld;
...
bool DlgFmncNavLoaini::TagIfi::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagDlgFmncNavLoainiIfi");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemDlgFmncNavLoainiIfi";
...
DlgFmncNavLoaini::TagImp::TagImp(
			const string& CptPrg
			, const string& ButRun
			, const string& ButSto
		) : Block() {
...
bool DlgFmncNavLoaini::TagImp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagDlgFmncNavLoainiImp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemDlgFmncNavLoainiImp";
...
DlgFmncNavLoaini::TagLfi::TagLfi(
			const string& Dld
		) : Block() {
	this->Dld = Dld;
...
bool DlgFmncNavLoaini::TagLfi::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagDlgFmncNavLoainiLfi");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemDlgFmncNavLoainiLfi";
...
DlgFmncNavLoaini::DpchAppData::DpchAppData(
			const string& scrJref
			, ContIac* contiac
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPDLGFMNCNAVLOAINIDATA, scrJref) {
...
void DlgFmncNavLoaini::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppDlgFmncNavLoainiData");
...
DlgFmncNavLoaini::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const uint ixVDoImp
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPDLGFMNCNAVLOAINIDO, scrJref) {
...
void DlgFmncNavLoaini::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppDlgFmncNavLoainiDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
DlgFmncNavLoaini::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGDLGFMNCNAVLOAINIDATA) {
...
void DlgFmncNavLoaini::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngDlgFmncNavLoainiData");
	else
		basefound = checkXPath(docctx, basexpath);
