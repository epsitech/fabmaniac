/**
  * \file CrdFmncBil.cpp
  * API code for job CrdFmncBil (implementation)
...
bool CrdFmncBil::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncBil");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncBil";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdBil", MtxCrdBil)) add(MTXCRDBIL);
	};
...
set<uint> CrdFmncBil::ContInf::comm(
	...
	if (MtxCrdBil.compare(comp->MtxCrdBil) == 0) insert(items, MTXCRDBIL);
...
bool CrdFmncBil::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppFmncBil");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppFmncBil";
...
set<uint> CrdFmncBil::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdFmncBil::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdFmncBil::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncBil");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncBil";
...
bool CrdFmncBil::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncBil");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncBil";
...
CrdFmncBil::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCBILDO, scrJref) {
...
void CrdFmncBil::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncBilDo");
...
CrdFmncBil::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCBILDATA) {
...
void CrdFmncBil::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncBilData");
	else
		basefound = checkXPath(docctx, basexpath);
