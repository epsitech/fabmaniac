/**
  * \file PnlFmncSkiHeadbar.cpp
  * API code for job PnlFmncSkiHeadbar (implementation)
...
PnlFmncSkiHeadbar::StgInf::StgInf(
...
bool PnlFmncSkiHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfFmncSkiHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfFmncSkiHeadbar";
...
set<uint> PnlFmncSkiHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlFmncSkiHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlFmncSkiHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncSkiHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncSkiHeadbar";
...
PnlFmncSkiHeadbar::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCSKIHEADBARDATA) {
};
...
void PnlFmncSkiHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncSkiHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
