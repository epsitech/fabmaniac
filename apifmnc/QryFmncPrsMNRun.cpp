/**
  ...
  * API code for job QryFmncPrsMNRun (implementation)
...
/******************************************************************************
 class QryFmncPrsMNRun::StatApp
 ******************************************************************************/

QryFmncPrsMNRun::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncPrsMNRun::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncPrsMNRun");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncPrsMNRun";
...
set<uint> QryFmncPrsMNRun::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncPrsMNRun::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncPrsMNRun::StatShr
 ******************************************************************************/

QryFmncPrsMNRun::StatShr::StatShr(
			const uint ntot
...
bool QryFmncPrsMNRun::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncPrsMNRun");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncPrsMNRun";
...
set<uint> QryFmncPrsMNRun::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryFmncPrsMNRun::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncPrsMNRun::StgIac
 ******************************************************************************/

QryFmncPrsMNRun::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryFmncPrsMNRun::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryFmncPrsMNRun");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryFmncPrsMNRun";
...
void QryFmncPrsMNRun::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryFmncPrsMNRun";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryFmncPrsMNRun";
...
set<uint> QryFmncPrsMNRun::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryFmncPrsMNRun::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
