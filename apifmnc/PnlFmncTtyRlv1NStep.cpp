/**
  ...
  * API code for job PnlFmncTtyRlv1NStep (implementation)
...
bool PnlFmncTtyRlv1NStep::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfFmncTtyRlv1NStep");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfFmncTtyRlv1NStep";
...
PnlFmncTtyRlv1NStep::StatApp::StatApp(
...
bool PnlFmncTtyRlv1NStep::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppFmncTtyRlv1NStep");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppFmncTtyRlv1NStep";
...
set<uint> PnlFmncTtyRlv1NStep::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlFmncTtyRlv1NStep::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlFmncTtyRlv1NStep::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrFmncTtyRlv1NStep");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrFmncTtyRlv1NStep";
...
bool PnlFmncTtyRlv1NStep::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagFmncTtyRlv1NStep");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemFmncTtyRlv1NStep";
...
PnlFmncTtyRlv1NStep::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryFmncTtyRlv1NStep::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTTYRLV1NSTEPDATA, scrJref) {
...
void PnlFmncTtyRlv1NStep::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncTtyRlv1NStepData");
...
PnlFmncTtyRlv1NStep::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppFmnc(VecFmncVDpch::DPCHAPPFMNCTTYRLV1NSTEPDO, scrJref) {
...
void PnlFmncTtyRlv1NStep::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppFmncTtyRlv1NStepDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/fmnc");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlFmncTtyRlv1NStep::DpchEngData::DpchEngData() : DpchEngFmnc(VecFmncVDpch::DPCHENGFMNCTTYRLV1NSTEPDATA) {
...
void PnlFmncTtyRlv1NStep::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngFmncTtyRlv1NStepData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryFmncTtyRlv1NStep::StatApp();
		statshrqry = QryFmncTtyRlv1NStep::StatShr();
