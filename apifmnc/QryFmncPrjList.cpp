/**
  ...
  * API code for job QryFmncPrjList (implementation)
...
/******************************************************************************
 class QryFmncPrjList::VecVOrd
 ******************************************************************************/

uint QryFmncPrjList::VecVOrd::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);

	if (s.compare("tit") == 0) return TIT;
	else if (s.compare("ste") == 0) return STE;

	return(0);
};

string QryFmncPrjList::VecVOrd::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class QryFmncPrjList::StatApp
 ******************************************************************************/

QryFmncPrjList::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncPrjList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncPrjList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncPrjList";
...
set<uint> QryFmncPrjList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncPrjList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncPrjList::StatShr
 ******************************************************************************/

QryFmncPrjList::StatShr::StatShr(
			const uint ntot
...
bool QryFmncPrjList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncPrjList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncPrjList";
...
set<uint> QryFmncPrjList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryFmncPrjList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryFmncPrjList::StgIac
 ******************************************************************************/

QryFmncPrjList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryFmncPrjList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryFmncPrjList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryFmncPrjList";
...
void QryFmncPrjList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryFmncPrjList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryFmncPrjList";
...
set<uint> QryFmncPrjList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryFmncPrjList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
