/**
  ...
  * API code for job QryFmncRunMNFab (implementation)
...
QryFmncRunMNFab::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncRunMNFab::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncRunMNFab");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncRunMNFab";
...
set<uint> QryFmncRunMNFab::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncRunMNFab::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryFmncRunMNFab::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncRunMNFab");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncRunMNFab";
