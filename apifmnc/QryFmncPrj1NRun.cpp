/**
  ...
  * API code for job QryFmncPrj1NRun (implementation)
...
QryFmncPrj1NRun::StatApp::StatApp(
			const uint firstcol
...
bool QryFmncPrj1NRun::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryFmncPrj1NRun");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryFmncPrj1NRun";
...
set<uint> QryFmncPrj1NRun::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryFmncPrj1NRun::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryFmncPrj1NRun::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryFmncPrj1NRun");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryFmncPrj1NRun";
